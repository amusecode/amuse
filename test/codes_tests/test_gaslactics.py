...
from amuse.community.gaslactics.interface import GaslactICsInterface, GaslactICs

# Change the default for some GaslactICs(-Interface) keyword arguments:
#default_options = dict()
default_options = dict(redirection = "none")

class GaslactICsInterfaceTests(TestWithMPI):
    
    def test1(self):
        print "Testing GaslactICsInterface initialization"
        instance = GaslactICsInterface(**default_options)
    ...
    def test2(self):
        print "Testing GaslactICsInterface parameters"
        instance = GaslactICsInterface(**default_options)
        self.assertEqual(instance.initialize_code(), 0)
        ...
        self.assertEquals(instance.set_halo_virial_radius(250.0), 0)
        ...
        self.assertEquals([250.0, 0], instance.get_halo_virial_radius().values())
    ...
    def slowtest3(self):
        print "Testing GaslactICsInterface generate_particles"
...
class GaslactICsTests(TestWithMPI):
     
    default_unit_converter = nbody_system.nbody_to_si(1.0 | units.kpc, 1.0e6 | units.MSun)
   
    def test1(self):
        print "Testing GaslactICs initialization"
        instance = GaslactICs(**default_options)
    ...
    def test2(self):
        print "Testing GaslactICs parameters (with unit converter)"
        instance = GaslactICs(self.default_unit_converter, **default_options)
        instance.initialize_code()
        
        for par, value in [('halo_do_center_flag', True), 
        ...
        for par, value in [('generate_halo_flag', 3), ('generate_disk_flag', 3), 
                ('generate_bulge_flag', 3), ('number_of_grid_intervals', 90000), ('order_of_multipole_expansion', 12), 
                ('number_of_radial_steps_correction_fns_disk_df', 200),
                ('number_of_iterations_disk_df', 12), ('halo_number_of_particles', 200000), 
                ('bulge_number_of_particles', 50000), ('disk_number_of_particles', 100000), 
                ('halo_random_seed', -234), ('bulge_random_seed', -123), ('disk_random_seed', -521)]:
        ...
        for par, value in [('halo_virial_radius', 40.0 | nbody_system.length), 
                ('halo_density_scale', 0.01 | nbody_system.density), 
                ('halo_scale_radius', 13.7 | nbody_system.length)]:
    ...
    def test3(self):
        print "Testing GaslactICs parameters (nbody units, no converter)"
        instance = GaslactICs(**default_options)
        instance.initialize_code()
    ...
    def slowtest4(self):
        print "Testing GaslactICs generate_particles"
        n_particles_halo = 100
        n_particles_bulge = 100
        n_particles_disk = 100
        number_of_particles = n_particles_disk + n_particles_bulge + n_particles_halo
    ...
    def xtest6(self):
        print "Testing GaslactICs generate_particles: generate multiple sets"
        ...
        # GaslactICs' random-number generator is re-seeded with 'halo_random_seed'
    ...
    def xtest7(self):
        print "Testing GaslactICs state"
        number_of_particles = 1000
