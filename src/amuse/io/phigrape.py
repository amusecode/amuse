...
index   mass    x y z   vx vy vz   # N data lines where
                                   # index is a running particle index starting
                                   # at 0!
...
class Particles2Inp(object):
    ...
    def convert_to_inp(self, particles, filename):
        ...
        for i in range(self.no_particles):
            self.string += LINEFORMAT16.format(
                    particle_index=i, mass=masses[i], x=positions[i][0],
                    y=positions[i][1], z=positions[i][2], vx=velocities[i][0],
                    vy=velocities[i][1], vz=velocities[i][2])
    ...
    def read_to_ram(self, inputfile):
        ...
        for i, line in enumerate(lines):
            if i < 3:
                ...
                n, m, x, y, z, vx, vy, vz = (
                        self.get_values_from_phase_line(line))
