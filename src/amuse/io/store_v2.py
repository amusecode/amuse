try:
    ...
    import warnings
    warnings.warn(
            "could not load h5py, hdf5 files not supported",
            ImportWarning,
            )
    h5py = None
    ...
    def new_attribute(cls, name, shape, input, group):
        if is_quantity(input):
            ...
            dataset = group.create_dataset(
                    name, shape=shape, dtype=input.number.dtype)
            dataset.attrs["units"] = (
                    input.unit.to_simple_form().reference_string()
                    )
            return HDF5VectorQuantityAttribute(name, dataset, input.unit)
        elif hasattr(input, 'as_set'):
            raise Exception(
                    """adding a linked attribute to a set stored in a HDF file
                    is not supported, alternative is to copy the set and save
                    it""")
...
class HDF5VectorQuantityAttribute(HDF5Attribute):
    ...
    def set_values(self, indices, values):
        try:
            ...
            if not is_quantity(values):
                raise ValueError(
                        "Tried to put a non quantity value in a quantity")
            else:
                raise
    ...
    def increase_to_length(self, newlength):
        ...
        values = numpy.empty(
                shape=self.dataset.shape, dtype=self.dataset.dtype)
        values[:] = self.dataset[:]

        parent = self.dataset.parent
        del parent[self.name]
        self.dataset = parent.create_dataset(
                self.name, shape=newshape, dtype=values.dtype)
    ...
    def copy(self):
        ...
        raise NotImplementedError(
                "Copy of an attribute in a hdf5 file is not implemented")
    ...
    def remove_indices(self, indices):
        ...
        del parent[self.name]
        self.dataset = parent.create_dataset(
                self.name, shape=values.shape, dtype=values.dtype)
        self.dataset[:] = values[:]
...
class HDF5UnitlessAttribute(HDF5Attribute):
    ...
    def copy(self):
        ...
        # return result
        raise NotImplementedError(
                "Copy of an attribute in hdf5 is not implemented")
...
class HDF5LinkedAttribute(HDF5Attribute):
    ...
    def get_values(self, indices):
        ...
        if len(shape) == 0:
            ...
            result = self.convert_to_object(
                    kinds, references, keys, grid_indices)
        else:
            for index in numpy.ndindex(*shape):
                ...
                kind = kinds[index]
                result[index] = self.convert_to_object(
                        kind, reference, keys[index], grid_indices[index])
    ...
    def convert_to_object(self, kind, reference, key, grid_index):
        if kind == 0:
            ...
            mapping_from_groupid_to_set = (
                    self.loader.mapping_from_groupid_to_set)
    ...
    def increase_to_length(self, newlength):
        ...
        self.keys = parent.create_dataset(
                'keys', shape=newshape, dtype=values.dtype)
        ...
        self.masked = parent.create_dataset(
                'masked', shape=newshape, dtype=values.dtype)
...
class HDF5GridAttributeStorage(AttributeStorage):
    ...
    def add_particles_to_store(self, keys, attributes=[], quantities=[]):
        raise exceptions.AmuseException(
                "adding points to the grid is not implemented")
...
class StoreHDF(object):
    ...
    def __init__(
            self, filename, append_to_file=True, open_for_writing=True,
            copy_history=False, return_working_copy=False):
        if h5py is None:
            raise exceptions.AmuseException(
    ...
    def store_sets(self, sets, names, extra_attributes={}):
        ...
        for x, name in zip(sets, names):
            if hasattr(x, 'shape'):
                self.store_grid(
                        x, extra_attributes, parent=self.named_group(name))
            else:
                self.store_particles(
                        x, extra_attributes, self.named_group(name),
                        mapping_from_setid_to_group, links_to_resolve)
    ...
    def store_particles(
            self, particles, extra_attributes={}, parent=None,
            mapping_from_setid_to_group={}, links=[]):
        if parent is None:
        ...
        group.attrs["class_of_the_particles"] = pickle_to_string(
                particles._factory_for_new_collection())

        # keys = particles.get_all_keys_in_store()
        # dataset = group.create_dataset("keys", data=keys)
        self.hdf5file.flush()
        self.store_collection_attributes(
                particles, group, extra_attributes, links)
    ...
    def store_grid(
            self, grid, extra_attributes={}, parent=None,
            mapping_from_setid_to_group={}, links=[]):
        if parent is None:
            parent = self.data_group()
        ...
        group.attrs["class_of_the_container"] = pickle_to_string(
                grid._factory_for_new_collection())
        ...
        for unresolved_link in links:
            ...
            if not unresolved_link.is_resolved():
                if not id(unresolved_link.linked_set) in seen_sets_by_id:
                    sets_to_store.append(
                            [unresolved_link.group, unresolved_link.linked_set]
                            )
    ...
    def store_values(self, container, group, links=[]):
        ...
        for attribute, quantity in zip(
            ...
            if is_quantity(quantity):
                ...
                dataset.attrs["units"] = (
                        quantity.unit.to_simple_form().reference_string())
            elif isinstance(quantity, LinkedArray):
                self.store_linked_array(
                        attribute, attributes_group, quantity, group, links)
            else:
                dtype = numpy.asanyarray(quantity).dtype
                if dtype.kind == 'U':
                    dataset = attributes_group.create_dataset(
                            attribute, data=numpy.char.encode(
                                quantity, 'UTF-32BE')
                            )
                    dataset.attrs["units"] = "UNICODE"
                else:
    ...
    def store_linked_array(
            self, attribute, attributes_group, quantity, group, links):
        ...
        for index in numpy.ndindex(*shape):
            ...
            if isinstance(object, GridPoint):
                max_len_grid_indices = max(
                        len(object.index), max_len_grid_indices)
    ...
    def store_collection_attributes(
            self, container, group, extra_attributes, links):
    ...
    def load(self):
        if (
                self.data_group(False) is not None
                and len(self.data_group(False)) > 0
                ):
    ...
    def load_particles_from_group(self, group):
        try:
            class_of_the_container = unpickle_from_string(
                    group.attrs["class_of_the_particles"])
        except:
        ...
        if len(keys) == 0:
            ...
            particles = class_of_the_container(is_working_copy=False)
            particles._private.attribute_storage = HDF5AttributeStorage(
                    keys, group, self)
    ...
    def load_grid_from_group(self, group):
        try:
            class_of_the_container = unpickle_from_string(
                    group.attrs["class_of_the_container"])
        except:
        ...
        container._private.attribute_storage = HDF5GridAttributeStorage(
                shape, group, self)
    ...
    def load_from_group(self, group):
        ...
        if container_type == 'particles':
            ...
            raise Exception(
                    'unknown container type in file {0}'.format(
                        container_type
                        )
                    )
