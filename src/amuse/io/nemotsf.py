...
class Particles2Tsf(object):
    ...
    def convert_to_string(self, particles, converter=None):
        ...
        self.time = (
                particles.get_timestamp()
                or (0.0 | nbody_system.time)
                ).value_in(nbody_system.time)
...
class Tsf2Particles(object):
    ...
    def read_to_ram(self, string):
        ...
        timestamp_index = [
                ...
                if 'double Time' in oneline
                ][0]
        self.timestamp = float(
                lines[timestamp_index].strip().split(' ')[2]
                ) | nbody_system.time

        start_masses = [
                i
                for i, oneline in enumerate(lines)
                if 'double Mass' in oneline
                ][0]
        start_phasespace = [
                i
                for i, oneline in enumerate(lines)
                if 'double PhaseSpace' in oneline
                ][0]
        # massline_numbers = self.return_numbers_in_brackets(
        #         lines[start_masses])
        phaseline_numbers = self.return_numbers_in_brackets(
                lines[start_phasespace])
        ...
        self.masses = self.return_numbers_from_paragraph(
                lines, start_masses, start_phasespace)
        self.phases = numpy.reshape(
                self.return_numbers_from_paragraph(
                    lines, start_phasespace, len(lines)),
...
class NemoFileFormatProcessor(base.FullTextFileFormatProcessor):
    ...
    def nbody_to_si_converter(self):
        """NEMO datafiles store nbody data, provide a converter to store si
        data (None means no converter)"""
        return None
