...
class TableFormattedText(base.FileFormatProcessor):
    ...
    def _get_attribute_names(self):
        if self.set is None:
            return map(
                    lambda x: "col({0})".format(x),
                    range(len(self.quantities))
                    )
        else:
    ...
    def attribute_dtypes(self):
        """
        List of the data types of the attributes. If not given, the data types
        will be float64.
        """
    ...
    def _new_converter_from_string_to_dtype(self, dtype):
        ...
        if kind == 'f':
            ...
            return lambda string_value: (
                    string_value == 'True'
                    or string_value == 'true'
                    )
        else:
    ...
    def header_prefix_string(self):
        """
        Lines starting with this character will be handled as part of the
        header
        """
        return '#'
    ...
    def read_header(self):
        while (
                not self.cursor.is_at_end()
                and self.cursor.line().startswith(self.header_prefix_string)
                ):
    ...
    def read_rows(self):
        ...
        self.set = None
        string_converters = map(
                self._new_converter_from_string_to_dtype,
                self.attribute_dtypes)
        units_with_dtype = map(
                core.unit_with_specific_dtype,
                self.attribute_types,
                self.attribute_dtypes)

        while (
                not self.cursor.is_at_end()
                and not self.cursor.line().startswith(
                    self.footer_prefix_string)
                ):
                ...
                if self.key_in_column >= 0:
                    if len(columns) != len(self.attribute_names) + 1:
                        raise base.IoException(
                            "Number of values on line '{0}' is {1}, expected {2}".format(
                                self.cursor.line(),
                                len(columns),
            ...
            if number_of_particles >= self.maximum_number_of_lines_buffered:
                quantities = map(
                    lambda value, unit, dtype: unit.new_quantity(value) if unit is not None else (numpy.asarray(value, dtype=dtype) if dtype is not None else value),
                    values,
                    units_with_dtype,
                    self.attribute_dtypes
                )
                ...
                keys = []
                values = map(
                        lambda x: [], range(len(self.attribute_names)))
    ...
    def read_footer(self):
        while (
                not self.cursor.is_at_end()
                and self.cursor.line().startswith(self.footer_prefix_string)
                ):
            self.read_footer_line(
                    self.cursor.line()[len(self.footer_prefix_string):]
                    )
            self.cursor.forward()
    ...
    def write_rows(self):
        ...
        while offset < max_row:
            ...
            numbers = map(
                    lambda quantity, unit: quantity[offset:offset+block_size] if unit is None else quantity[offset:offset+block_size].value_in(unit), quantities, units)
            ...
            for i in range(len(columns[0])):
                ...
                if self.key_in_column >= 0:
                    row.insert(
                            self.key_in_column,
                            self.convert_long_to_string(self.keys[i+offset])
                            )
    ...
    def write_row(self, row):
        units = self.attribute_types
        row = map(
                lambda quantity, unit: quantity if unit is None else quantity.value_in(unit), row, units)
    ...
    def header_lines(self):
        ...
        if self.has_amuse_header:
            ...
            for i, (x, unit) in enumerate(
                    zip(self.attribute_names, self.attribute_types)):
                column = i
                if self.key_in_column >= 0:
                ...
                result.append(
                        'COL:{0}:{1}:{2}:{3}:{4}'.format(
                            column, x, 'on', unitstring, description
                            )
                        )
    ...
    def quantities(self):
        ...
        if self.set is None:
            ...
            if len(self.set) == 0:
                ...
                return map(
                        lambda x: getattr(self.set, x), self.attribute_names)
    ...
    def maximum_number_of_lines_buffered(self):
        """The maximum number of lines to convert and write out in one go,
        larger values may be a little faster but will use a lot more memory"""
        return 10000

    @base.format_option
    def is_precise(self):
        """Output most precise text output for floats (9 digitst) and doubles
        (17 digits)"""
        return False
    ...
    def convert_float_to_unit(self, floats):
        ...
        print(
                floats,
                int(floats[1]) == -1,
                numpy.all(numpy.asarray(floats[2:]) == 0.0),
                numpy.asarray(floats[2:]) == 0.0,
                )
        if int(floats[1]) == -1 and numpy.all(
                numpy.asarray(floats[2:]) == 0.0
                ):
            return None
        factor = floats[0]
...
class CsvFileText(TableFormattedText):
    ...
    def convert_csv_string_to_unit(self, csv_string):
        return [
                self.convert_string_to_unit(sub)
                for sub in csv_string.split(self.column_separator)
                ]
    ...
    def read_header_line(self, line):
        if self.attribute_names:
            ...
            self.attribute_names = [
                    sub.strip()
                    for sub in line.split(self.column_separator)
                    ]
    ...
    def header_lines(self):
        ...
        if len(self.comments) > 0:
            result.extend(
                    map(
                        lambda x: self.header_prefix_string + x, self.comments
                        )
                    )
        result.append(self.column_separator.join(self.attribute_names))
                    ...
                    self.column_separator.join(
                        [
                            ...
                            if one_unit is None
                                ...
                                one_unit.to_simple_form().reference_string()
                                for one_unit in self.attribute_types
                                )
                            ]
                        )
                    )
            result.append(
                    self.column_separator.join(map(str, self.attribute_types))
                    )
        return result
...
class Athena3DText(TableFormattedText):
    ...
    def read_attribute_names_from_line(self, line):
        ...
        self.attribute_names = self.convert_dictionary_to_array(
                mapping_from_column_index_to_name)
