...
class HDF5LinkedAttribute(HDF5Attribute):
    ...
    def increase_to_length(self, newlength):
        ...
        values = numpy.empty(
                shape=self.masked.shape,
                dtype=self.masked.dtype,
                )
        values[:] = self.masked[:]
        parent = self.group
        del parent['masked']
...
class HDF5GridAttributeStorage(AttributeStorage):
    ...
    def get_values_in_store(self, indices, attributes):
        ...
        for attribute in attributes:
            ...
            # if indices is None:
            #     selected_values = numpy.ndarray(
            #             shape=self.shape, dtype = values_vector.dtype)
            #     values_vector.read_direct(selected_values)
            # else:
            #     # selection = h5py.selections.PointSelection(
            #     #         values_vector.shape)
    ...
    def set_values_in_store(self, indices, attributes, quantities):
        for attribute, quantity in zip(attributes, quantities):
            if attribute in self.attributesgroup:
                ...
                dataset = self.attributesgroup.create_dataset(
                        attribute,
                        shape=self.shape,
                        dtype=quantity.number.dtype,
                        )
                dataset["unit"] = quantity.unit.to_simple_form(
                        ).reference_string()
...
class StoreHDF(object):
            ...
            for x in set(sets_to_store):
                if hasattr(x, 'shape'):
                    self.store_grid(
                            x,
                            extra_attributes,
                            parent=self.unnamed_references_group(),
                            )
                else:
                    self.store_particles(
                            x,
                            {},
                            self.unnamed_references_group(),
                            mapping_from_setid_to_group,
                            links_to_resolve,
                            )
    ...
    def load(self):
        if self.PARTICLES_GROUP_NAME not in self.hdf5file:
    ...
    def load_particles_from_group(self, group):
        class_of_the_particles = unpickle_from_string(
