...
class HDF5FileFormatProcessor(base.FileFormatProcessor):
    ...
    def __init__(
            self, filename=None, set=None, format=None, append_to_file=True):
    ...
    def store(self):
        ...
        if self.version == '1.0':
            ...
            if not processor.is_correct_version():
                raise Exception(
                        "You are trying to append to a file that was not\
                        written in version 1.0 format")
        else:
            processor = store_v2.StoreHDF(
                self.filename,
                self.append_to_file,
            ...
            if not processor.is_correct_version():
                raise Exception("You are trying to append to a file that \
                        was written in version 1.0 format")
        try:
            if len(self.names) > 0:
                return processor.store_sets(
                        self.set, self.names, self.extra_attributes)
            else:
    ...
    def allow_writing(self):
        """If set to True, data can be written to the file, even if
        read_set_from_file is used"""
        return False
