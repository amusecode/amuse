...
class UnsupportedFormatException(IoException):
    ...
    """
    formatstring = "You tried to load or save a file with fileformat '{0}', \
...
class CannotSaveException(IoException):
    ...
    """
    formatstring = "You tried to save a file with fileformat '{0}', but this\
            format is not supported for writing files"
...
class CannotLoadException(IoException):
    """Raised when the given format cannot read data (only saving of data is\
            supported for the format)
    """
    formatstring = "You tried to load a file with fileformat '{0}', but this\
            format is not supported for reading files"
...
def _get_processor_factory(format):
    if isinstance(format, basestring):
        if format not in registered_fileformat_processors:
...
def read_set_from_file(
    ...
    """
    if (
            ('stream' not in format_specific_keyword_arguments)
...
class ReportTable(object):
    ...
    :argument attribute_names: list of the names of for the values
        (used in the header of the file and when using add_row with keyword
        parameters)
    ...
    Example usage::
        ...
        report = ReportTable(
            ...
            attribute_names=('age', 'temperature_at_time',
            'luminosity_at_time')
        )
        report.add_row(particle.age, particle.temperature_at_time,
...
def add_fileformat_processor(class_of_the_format):
    ...
    :func:`FileFormatProcessor.register`
    """
...
class FortranFileFormatProcessor(BinaryFileFormatProcessor):
    ...
    def read_fortran_block(self, file):
        ...
        if(length_of_block_after != length_of_block):
            raise IoException(
                    "Block is mangled sizes don't match before: {0},\
                            after: {1}".format(
                                length_of_block,
                                length_of_block_after)
                            )
        return result
