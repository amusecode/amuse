...
class GadgetFileFormatProcessor(base.FortranFileFormatProcessor):
    ...
    def ids_are_keys(self):
        """Set to True if the file contains correct keys. Set to False to
        generate the keys in amuse and prove an id attribute for the id's in
        the gadget file"""
        return True
    ...
    def return_header(self):
        """Set to True to return both the particles and the header from the
        gadget file"""
        return False

    @base.format_option
    def write_header_from(self):
        """Pass a namedtuple to store non-default values in the header of the
        gadget file"""
        return None

    @base.format_option
    def convert_gadget_w_to_velocity(self):
        """Set to True to convert the w=sqrt(a)*dx/dt to (comoving) velocity in
    ...
    def load_header(self, file):
        ...
        values = struct.unpack(
                self.header_format, header_bytes[0:self.header_size])
    ...
    def sets_to_save(self):
        if isinstance(self.set, (tuple, list)):
            ...
            raise Exception(
                    """The Gadget binary file writer can write a particle set
                    or a list of sets but nothing else""")
    ...
    def store_header(self, file):
        self.default_header = dict(
                Massarr=list(
                    self.equal_mass_array.value_in(nbody_system.mass)
                    ),
                Time=0.0,
                Redshift=0.0,
                FlagSfr=0,
    ...
    def store_body(self, file):
        ...
        if number_of_gas_particles > 0:
            self.density = self.sets_to_save[0].rho.value_in(
                    nbody_system.density)
            self.hsml = self.sets_to_save[0].h_smooth.value_in(
                    nbody_system.length)
        else:
            self.density = None
            self.hsml = None
        ...
        if self.has_potential_energy:
            self.pot = self._pack_sets(
                    'potential_energy', nbody_system.energy)
        else:
            self.pot = None

        if self.has_acceleration:
            self.acc = self._pack_sets(
                    'acceleration', nbody_system.acceleration)
        else:
            self.acc = None
