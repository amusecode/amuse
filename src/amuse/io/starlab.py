    ...
    def convert_starlab_string_to_xml_string(self, string):
        ...
        if closepar != openpar:
            raise base.IoException(
                    """\nConversion failure, parenthesis mismatch. Return: no
...
class Xml2Particles(object):
    ...
    def __init__(self):
        ...
        self.translator = {
            ...
            'r': (
                'position',
                lambda x: self.convert2vec(x) | self.dynamics_length_units),
            'v': (
                'velocity',
                lambda x: self.convert2vec(x) | (
                    self.dynamics_length_units / self.dynamics_time_units)
                ),
            'a': (
                'acceleration',
                lambda x: self.convert2vec(x) | (
                    self.dynamics_length_units
                    / (self.dynamics_time_units ** 2)
                    )
                ),
            'pot': (
                'specific_potential',
                lambda x: float(x) | (
                    self.dynamics_length_units / self.dynamics_time_units) ** 2
                ),
    ...
    def _recursive_parse_node_into_particles(
            self, xml_node, particle_or_particleset, parent=None):
        node_list = xml_node.childNodes
        for subnode in node_list:
            if subnode.nodeType == subnode.ELEMENT_NODE:
                if subnode.tagName == "System":  # skip
                    self._recursive_parse_node_into_particles(
                            subnode, particle_or_particleset)
                    ...
                    self.copy_starlab_parameter_to_star(
                            key, value, particle_or_particleset)
    ...
    def walk_me(self, scl):
        ...
        for attrlistitem in scl.attriblist:
            ...
            elif attribute not in ["nsisters"]:
...
class Particles2Dyn(object):
    ...
    def convert_to_string(self, particles):
        ...
        # # Timestamp is only saved if not None
        timestamp = particles.get_timestamp() or (0 | nbody_system.time)
        # Timestamp is always saved
        if timestamp is not None:
            lines.append("(Dynamics")
            lines.append(
                    prefix
                    + "system_time  =  "
                    + str(timestamp.value_in(nbody_system.time))
                    )
            lines.append(")Dynamics")
        for index, x in enumerate(particles):
            ...
            lines.append(
                    prefix
                    + "m = "
                    + float_format(x.mass.value_in(nbody_system.mass))
                    )
...
class StarlabFileFormatProcessor(base.FullTextFileFormatProcessor):
    ...
    def return_children(self):
        """If True returns the children of the root node, if False returns the
        root node (defaults to True)"""
        return True

    @base.format_option
    def nbody_to_si_converter(self):
        """Starlab datafiles store stellar dynamics properties in scaled nbody
        values, provide a converter to store si data (defaults to None).
        Value None means no converter, or use scaling values provided in the
        file"""
        return None

    @base.format_option
    def dynamics_mass_units(self):
        """The m field in the dynamics section of a starlab file can be in MSun
        or in scaled units, defaults to scaled units (nbody_system.mass).
        When set to scaled units, AMUSE will convert the units if scaling
        parameters are also given in the file. See the `must_scale` option
        to turn this scaling off"""
        return nbody_system.mass

    @base.format_option
    def dynamics_time_units(self):
        """The time fields in the dynamics section of a starlab file can be in
        Myr or in scaled units, defaults to scaled units (nbody_system.time).
        When set to scaled units, AMUSE will convert the units if scaling
        parameters are also given in the file. See the `must_scale` option
        to turn this scaling off.
        """
    ...
    def dynamics_length_units(self):
        """The length fields in the dynamics section of a starlab file can be
        in parsec or in scaled units, defaults to scaled units
        (nbody_system.length). When set to scaled units, AMUSE will convert the
        units if scaling parameters are also given in the file. See the
        `must_scale` option to turn this scaling off.
        """
        return nbody_system.length

    @base.format_option
    def must_scale(self):
        """If True use the scaling values from the file, if False do not scale
        the stellar dynamics properties.  Only used when no nbody to si
    ...
    def return_converter(self):
        """If True also return the converter when reading a set using
        read_set_from_file """
        return False
