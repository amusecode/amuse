...
class TestMPIInterface(TestWithMPI):
    ...
    def test2(self):
        ...
        result = sse.evolve_star(
            ...
            new_state.mass,
            new_state.radius,
            new_state.luminosity,
            new_state.core_mass,
            new_state.core_radius,
            new_state.envelope_mass,
            new_state.envelope_radius,
            new_state.spin,
            new_state.epoch,
            new_state.t_ms,
            new_state.sse_age,
        ...
        (
            updated_state.stellar_type,
            updated_state.zams_mass,
            updated_state.mass,
            updated_state.radius,
            updated_state.luminosity,
            updated_state.core_mass,
            updated_state.core_radius,
            updated_state.envelope_mass,
            updated_state.envelope_radius,
            updated_state.spin,
            updated_state.epoch,
            updated_state.t_ms,
            updated_state.sse_age,
            updated_state.age,
        ) = result
        attributes = (
            "stellar_type",
    ...
    def test3(self):
        ...
        luminosity = (
            core_mass
        ) = core_radius = envelope_mass = envelope_radius = spin = epoch = t_ms = [
            0.0,
            0.0,
            0.0,
        ]
...
class TestSse(TestWithMPI):
    def test1(self):
        ...
        for result, expected in zip(results, masses):
            self.assertAlmostEqual(
                result[1].value_in(units.MSun), expected.value_in(units.MSun), 3
            )
    ...
    def test2(self):
        ...
        self.assertAlmostEqual(
            sse.particles[0].temperature.value_in(units.K), 4221.0, 0
        )
    ...
    def test6(self):
        ...
        masses = [0.5, 1.0, 2.0, 5.0, 10.0, 30.0] | units.MSun
    ...
    def test7(self):
        ...
        for star in stars:
            ...
            from_stellar_evolution_to_model = (
                stellar_evolution.particles.new_channel_to(star.as_set())
            )
    ...
    def test10(self):
        ...
        stellar_evolution.particles.add_particles(stars)
        self.assertEqual(
            stellar_evolution.particles._factory_for_new_collection(), Particles
        )
    ...
    def test11(self):
        ...
        self.assertAlmostEqual(
            instance.particles.time_step, [550.1565, 58.2081, 18.8768] | units.Myr, 3
        )
        self.assertAlmostEqual(
            instance.particles.radius,
            [0.8882494502, 1.610210385, 1.979134445] | units.RSun,
        )
        ...
        self.assertAlmostEqual(
            instance.particles.age, [18.8768, 18.8768, 18.8768] | units.Myr, 3
        )
        self.assertAlmostEqual(
            instance.particles.time_step, [550.1565, 58.2081, 18.8768] | units.Myr, 3
        )
        ...
        self.assertAlmostEqual(
            instance.particles.age, [100.0, 100.0, 100.0] | units.Myr, 3
        )
        self.assertAlmostEqual(
            instance.particles.time_step, [550.1565, 58.2081, 18.8768] | units.Myr, 3
        )
    ...
    def test12(self):
        ...
        self.assertAlmostEqual(
            instance.particles[2].age, 0.0 | units.Myr
        )  # ... and rejuvenated.
    ...
    def test14a(self):
        ...
        self.assertAlmostRelativeEqual(
            se_stars[0].luminosity, se_stars[1].luminosity, 3
        )
        self.assertAlmostRelativeEqual(se_stars[0].radius, se_stars[1].radius, 3)
        self.assertAlmostRelativeEqual(
            se_stars[0].temperature, se_stars[1].temperature, 3
        )
        instance.stop()
    ...
    def test15(self):
        ...
        class notsorandom(object):
            def random(self, N):
                return numpy.array(range(N)) / (N - 1.0)
    ...
    def test17(self):
        ...
        self.assertAlmostEqual(
            instance.particles.age[:-1], 2 * time_steps[[0, 2, 3, 5, 6, 7, 9]]
        )
    ...
    def test18(self):
        ...
        sse_src_path = os.path.join(
            os.path.dirname(sys.modules[Sse.__module__].__file__), "src"
        )
        ...
        shutil.copy(
            os.path.join(sse_src_path, "evolve.in"), os.path.join(testpath, "evolve.in")
        )
        ...
        self.assertAlmostEqual(
            evolved_star.age, float(sse_final_result[0]) | units.Myr, 3
        )
        self.assertAlmostEqual(
            evolved_star.stellar_type,
            float(sse_final_result[1]) | units.stellar_type,
            3,
        )
        self.assertAlmostEqual(
            evolved_star.initial_mass, float(sse_final_result[2]) | units.MSun, 3
        )
        self.assertAlmostEqual(
            evolved_star.mass, float(sse_final_result[3]) | units.MSun, 3
        )
        self.assertAlmostEqual(
            evolved_star.luminosity, 10 ** float(sse_final_result[4]) | units.LSun, 3
        )
        self.assertAlmostEqual(
            evolved_star.radius, 10 ** float(sse_final_result[5]) | units.RSun, 3
        )
        self.assertAlmostRelativeEqual(
            evolved_star.temperature, 10 ** float(sse_final_result[6]) | units.K, 2
        )
        self.assertAlmostEqual(
            evolved_star.core_mass, float(sse_final_result[7]) | units.MSun, 3
        )
        self.assertAlmostEqual(
            evolved_star.convective_envelope_mass,
            float(sse_final_result[8]) | units.MSun,
            3,
        )
        self.assertAlmostEqual(
            evolved_star.epoch, float(sse_final_result[9]) | units.Myr, 3
        )
        self.assertAlmostEqual(
            evolved_star.spin, float(sse_final_result[10]) | units.yr**-1, 3
        )
    ...
    def test20(self):
        ...
        stars = instance.particles.add_particles(
            Particles(mass=[0.6, 1.0] | units.MSun)
        )
    ...
    def test21(self):
        ...
        self.assertAlmostRelativeEquals(
            mass_loss_wind, 1.703e-07 | units.MSun / units.yr, 3
        )
        ...
        self.assertAlmostRelativeEquals(
            stars[0].mass_loss_wind, 2.053e-07 | units.MSun / units.yr, 3
        )
    ...
    def test22(self):
        instance = Sse()
        stars = instance.particles.add_particles(
            Particles(mass=[1.0, 10.0] | units.MSun)
        )
