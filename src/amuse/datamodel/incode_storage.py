...
class ParticleGetAttributesMethod(ParticleMappingMethod):
    ...
    def check_arguments(self, storage, attributes_to_return, *indices):
        if len(indices[0]) > 1:
            if self.method_is_legacy and not (
                self.method.specification.can_handle_array
                or self.method.specification.must_handle_array
            ):
                raise Exception(
    ...
    def convert_return_value(self, return_value, storage, attributes_to_return):
        ...
        for value, attribute, isindex in zip(
            return_value, self.attribute_names, index_output_attributes
        ):
    ...
    def get_attribute_values_async(self, storage, attributes_to_return, *indices):
        ...
        async_request = self.method.asynchronous(
            *indices, **storage.extra_keyword_arguments_for_getters_and_setters
        )
...
class ParticleGetGriddedAttributesMethod(ParticleGetAttributesMethod):
    ...
    def get_attribute_values(self, storage, attributes_to_return, *indices):
        ...
        minmax_per_dimension = self.get_range_method(
            **storage.extra_keyword_arguments_for_getters_and_setters
        )
        ...
        try:
            return_value = self.method(
                *one_dimensional_arrays_of_indices,
                **storage.extra_keyword_arguments_for_getters_and_setters,
            )
        except:
            print((self.method))
        ...
        mapping_from_name_to_value = self.convert_return_value(
            return_value, storage, attributes_to_return
        )
...
class ParticleSetAttributesMethod(ParticleMappingMethod):
    ...
    def set_attribute_values(self, storage, attributes, values, *indices):
        ...
        (
            list_args,
            keyword_args,
        ) = self.convert_attributes_and_values_to_list_and_keyword_arguments(
            attributes, values
        )
    ...
    def set_attribute_values_async(
        self,
        storage,
        attributes,
        values,
    ...
    def convert_attributes_and_values_to_list_and_keyword_arguments(
        ...
        if len(missing_attributes) > 0:
            ...
            raise exceptions.MissingAttributesAmuseException(
                missing_attributes,
                "To add particles to this code you need to specify the {0}".format(
                    missing_attributes_string
                ),
            )
...
class ParticleSetGriddedAttributesMethod(ParticleSetAttributesMethod):
    ...
    def set_attribute_values_async(self, storage, attributes, values, *indices):
        ...
        async_request = self.method.asynchronous(
            *one_dimensional_arrays_of_args, **keyword_args
        )
        return async_request
...
class ParticleQueryMethod(object):
    ...
    def apply_normal(self, particles, *args, **kwargs):
        ...
        keys = particles._private.attribute_storage._get_keys_for_indices_in_the_code(
            indices
        )
    ...
    def apply_for_superset(self, particles, *args, **kwargs):
        ...
        for subset in particles._private.particle_sets:
            ...
            for index in indices:
                if (
                    index
                    in subset._private.attribute_storage.mapping_from_index_in_the_code_to_particle_key
                ):
                    keys.append(
                        subset._private.attribute_storage.mapping_from_index_in_the_code_to_particle_key[
                            index
                        ]
                    )
...
class ParticleSpecificSelectMethod(object):
    ...
    def apply_on_all(self, particles):
        all_indices = list(
            particles._private.attribute_storage.mapping_from_index_in_the_code_to_particle_key.keys()
        )
        ...
        for indices in lists_of_indices:
            keys = (
                particles._private.attribute_storage._get_keys_for_indices_in_the_code(
                    indices
                )
            )
...
class ParticleMethod(AbstractCodeMethodWrapper):
    ...
    def apply_on_all(self, particles, *list_arguments, **keyword_arguments):
        ...
        all_indices = list(
            storage.mapping_from_index_in_the_code_to_particle_key.keys()
        )
...
class ParticleSetSelectSubsetMethod(object):
    ...
    def __init__(
        self,
        method,
        set_query_arguments_method=None,
        get_number_of_particles_in_set_method=None,
        public_name=None,
    ):
        self.method = method
        self.set_query_arguments_method = set_query_arguments_method
        self.get_number_of_particles_in_set_method = (
            get_number_of_particles_in_set_method
        )
        self.public_name = public_name
...
class ParticlesAddedUpdateMethod(object):
    def __init__(
        ...
        get_id_of_added_particles_method=None,
    ):
...
class AbstractInCodeAttributeStorage(base.AttributeStorage):
    ...
    def select_getters_for(self, attributes):
        ...
        sorted_getters = sorted(
            self.getters, key=lambda x: len(x.attribute_names), reverse=True
        )
    ...
    def select_setters_for(self, attributes):
        ...
        if set_of_attributes:
            raise exceptions.AmuseException(
                "Cannot set attributes {0}".format(sorted(set_of_attributes))
            )
...
class InCodeAttributeStorage(AbstractInCodeAttributeStorage):
    ...
    def add_particles_to_store(self, keys, attributes=[], values=[]):
        ...
        if len(self.particle_keys) > 0:
            ...
            self.particle_keys = numpy.concatenate(
                (self.particle_keys, numpy.array(list(keys)))
            )
            self.code_indices = numpy.concatenate(
    ...
    def get_indices_of(self, keys):
        ...
        for particle_key in keys:
            try:
                indices_in_the_code.append(
                    self.mapping_from_particle_key_to_index_in_the_code[particle_key]
                )
    ...
    def get_values_in_store_async(self, indices_in_the_code, attributes):
        ...
        if len(getters) > 1:
            ...
            request = ASyncRequestSequence(
                new_request, args=(getters, attributes, indices_in_the_code)
            )
        else:
    ...
    def set_values_in_store_async(self, indices_in_the_code, attributes, values):
        ...
        if len(setters) > 1:
            ...
            request = ASyncRequestSequence(
                new_request, args=(setters, attributes, values, indices_in_the_code)
            )
        else:
            for setter in setters:
                request = setter.set_attribute_values(
                    self, attributes, values, indices_in_the_code
                )
        return request
    ...
    def _add_indices(self, indices):
        ...
        for i in indices:
            if i in self.mapping_from_index_in_the_code_to_particle_key:
                raise exceptions.AmuseException(
                    "adding an index '{0}' that is already managed, bookkeeping is broken".format(
                        i
                    )
                )
...
class InCodeGridAttributeStorage(AbstractInCodeAttributeStorage):
    ...
    def __init__(
        ...
        AbstractInCodeAttributeStorage.__init__(
            self,
            code_interface,
            setters,
            getters,
    ...
    def get_values_in_store(self, indices, attributes):
        ...
        for getter in self.select_getters_for(attributes):
            result = getter.get_attribute_values(
                self, attributes, *one_dimensional_array_of_indices
            )
            mapping_from_attribute_to_result.update(result)
        ...
        for attribute in attributes:
            ...
            if len(array_of_indices) == 0:
                ...
                value = returned_value.reshape(
                    array_of_indices[0].shape + returned_value.shape[1:]
                )
    ...
    def set_values_in_store(self, indices, attributes, quantities):
        ...
        if len(one_dimensional_array_of_indices) == 0:
            ...
            one_dimensional_values = [
                (x.reshape(-1) if is_quantity(x) else numpy.asanyarray(x).reshape(-1))
                for x in quantities
            ]

        for setter in self.select_setters_for(attributes):
            setter.set_attribute_values(
                self,
                attributes,
                one_dimensional_values,
...
class ParticleSpecificSelectSubsetMethod(object):
    ...
    def __init__(
        self, method, get_number_of_particles_in_set_method=None, public_name=None
    ):
        self.method = method
        self.public_name = public_name
        self.get_number_of_particles_in_set_method = (
    ...
    def apply_on_one(self, set, particle):
        ...
        if not self.get_number_of_particles_in_set_method is None:
            ...
            indices = self.method(
                [from_indices[0]] * number_of_particles_in_set,
                list(range(number_of_particles_in_set)),
            )
        else:
            index = self.method()
            indices = [index]
