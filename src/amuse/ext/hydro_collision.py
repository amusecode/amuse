...
class StellarEncounterInHydrodynamics(object):
    ...
    def backtrack_particles(self, particles):
        ...
        if self.verbose:
            logger.info("Particles at collision:")
            logger.info(particles)
            logger.info(
                "Backtrack particles to initial separation %s",
                initial_separation.as_string_in(units.RSun),
            )
        ...
        if self.verbose:
            logger.info(
                "Backtracking particles done. Initial conditions:"
            )
            logger.info(
                particles
            )
    ...
    def convert_stars(self, particles, stellar_evolution_code):
        ...
        if self.verbose:
            logger.info(
                "Converting stars of {0} to SPH models of {1} particles, respectively.".format(particles.mass, n_particles)
            )
        sph_models = (
        ...
        if self.verbose:
            logger.info(
                "Converting stars to SPH particles done"
            )
        if self.debug:
            logger.info(
                gas_particles
    ...
    def relax(self, sph_model):
        ...
        if self.verbose:
            logger.info(
                "Relaxing SPH model with {0} for {1} ({2} dynamical timescales).".format(
    ...
    def new_hop(self, particles):
        ...
        if self.debug:
            logger.info("Output of Hop is redirected to hop_out.log")
    ...
    def simulate_collision(self, gas_particles):
        ...
        if self.verbose:
            logger.info(
                "Simulating collision with {0} from {1} to {2}.".format(
                self.hydrodynamics.__name__, 
        ...
        while True:
            if self.encounter_is_over(gas_particles):
                ...
                if extra_steps_counter > self.extra_steps_when_encounter_is_over:
                    logger.info("Encounter is over and finished extra steps.")
                    break
                else:
                    logger.info(
                        "Encounter is over. Now performing step {0} out of {1} extra steps".format(
                            extra_steps_counter, self.extra_steps_when_encounter_is_over)
                    )
            else:
                extra_steps_counter = 0
            logger.info(
                "Continuing to {0}.".format((hydro.model_time + self.next_dt + self.begin_time).as_string_in(units.day))
            )
    ...
    def encounter_is_over(self, gas_particles):
        ...
        if len(stars) > 1:
            ...
            if self.debug:
                logger.info(
                    "System consists of {0} groups. The two most massive are: {1} and {2}.".format(len(stars), a.mass.as_string_in(units.MSun), b.mass.as_string_in(units.MSun))
                )
        ...
        if self.verbose:
            logger.info(
                "Encounter is over, {0} stars after encounter.".format(len(groups))
            )
        return True
    ...
    def analyze_particle_distribution(self, gas_particles):
        if self.verbose:
            logger.info(
                "Analyzing particle distribution using Hop"
            )
        if "density" in gas_particles.get_attribute_names_defined_in_store():
            if self.debug:
                logger.info(
                    "Using the original particles' density"
                )
            ...
            if self.debug:
                logger.info(
                    "Using Hop to calculate the density"
                )
            self.hop.particles.add_particles(gas_particles)
    ...
    def binary_will_collide(self, a, b):
        ...
        if self.verbose:
            logger.info(
                "Using Kepler to check whether the two stars will (re)collide."
            )
        ...
        if true_anomaly > 0.0 and eccentricity >= 1.0:
            if self.verbose:
                logger.info(
                    "Stars are on hyperbolic/parabolic orbits and moving away from each other, interaction is over."
                )
            return False
        ...
        if self.verbose:
            logger.info(
                "Stars {0} collide. Distance at periastron: {1}, sum of radii: {2}".format(
        ...
        if will_collide:
            ...
            if self.debug:
                logger.info(
                    "Time to collision: {0}, next_dt: {1}".format(
            ...
            if kepler.get_time() > 3 * self.dynamical_timescale and kepler.get_apastron() > 2.0 * self.initial_separation * (a.radius + b.radius):
                ...
                if -kepler.get_time() > 2.9 * self.dynamical_timescale: # If ~3 t_dyn have passed since the end of the collision
                    if self.verbose:
                        logger.info(
                            "~3 t_dyn have passed since the end of the collision -> skip to next collision"
                        )
    ...
    def evolve_with_kepler(self, hydro):
        ...
        for group, old_particle, new_particle in zip(groups, old_particles, new_particles):
            ...
            if self.verbose:
                logger.info(
                    "%s %s %s",
                    in_hydro.center_of_mass().as_quantity_in(units.RSun),
                    old_particle.position.as_quantity_in(units.RSun),
                    new_particle.position.as_quantity_in(units.RSun)
                )
