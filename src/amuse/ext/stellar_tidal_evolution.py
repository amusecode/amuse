...
class TidalEvolution(literature.LiteratureReferencesMixIn):
    ...
    def evolve_model(self, time):
        ...
        logger.info("Central_particle: %s", self.central_particle.Omega)
        self.current_time = time
        if len(interacting_bodies):
            logger.info("Post tidal interaction %i", len(interacting_bodies))
        if len(self.orbiters_with_error)>0:
            logger.info(
                "Error in N=%i orbiters.",
                len(self.orbiters_with_error),
            )
            logger.info(self.orbiters_with_error)
        ...
        if len(merged_orbiters)>0:
            logger.info("Merged orbiters N= %i", len(merged_orbiters))
            logger.info(merged_orbiters)
    ...
    def evolve_individual_orbiter(self, oi, time, M, R, Os):
        ...
        while current_time<time:
            ...
            if self.contains_nan(da):
                logger.info("NAN's detected in da %s", da)
                self.orbiters_with_error.add_particles(oi.as_set())
                break
            if self.contains_nan(de):
                logger.info("NAN's detected, de %s", de)
                self.orbiters_with_error.add_particles(oi.as_set())
                break
            if self.contains_nan(dO):
                logger.info("NAN's detectedm dO %s", dO)
                self.orbiters_with_error.add_particles(oi.as_set())
                break
...
class TestTidalInteraction(TestCase):
    def test_remove_orbiters(self):
        ...
        while star.stellar_type<He_WD:
            logger.info("T=%s", stellar.model_time)
            ...
            if len(tidal.orbiters_with_error)>0:
                logger.info(
                    "Remove orbiter with error: %i",
                    len(tidal.orbiters_with_error),
                )
            ...
            if len(tidal.all_merged_orbiters)>0:
                logger.info(
                    "Merged planets/asteroids: N=%i",
                    len(tidal.all_merged_orbiters),
                )
                logger.info(
                    "removed: %s", tidal.all_merged_orbiters
                )
                tidal.orbiters.remove_particles(tidal.all_merged_orbiters)
                planet.remove_particle(tidal.all_merged_orbiters)
            logger.info("Remaining orbiters: %s", tidal.orbiters)
            logger.info("N=%i", len(tidal.orbiters))
...
#    dt = 1|units.Myr
    ...
    logger.info(
        "current time=%s %s %s",
        tidal.current_time,
        tidal.orbiters.semimajor_axis,
        tidal.orbiters.eccentricity,
    )
