#---------------------------------------------------------------------------
# Script that uses galactic_model and the bridge in rotating coordinates    |
...
class RealisticEvolutionCluster(object):
    ...
    in an analytical potential using galactic_model.
    ...
    def evolution_of_the_cluster(self, cluster):
        ...
        while (self.time<= self.t_end-self.dt_bridge/2):
            ...
            # write data
            if ((time==2) or (time==100) or (time==150) or (time==250)):
...
if __name__ in('__main__','__plot__'):
    ...
    star_cluster.position += [-8.5,0,0] | units.kpc
    star_cluster.velocity += [0, 100, 0] | units.kms  
    
   
    # Construct the galactic model and make the evolution
    evolution= RealisticEvolutionCluster(simulation_time= 250|units.Myr)
