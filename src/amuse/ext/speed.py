...
def calculate_speed(range_of_number_of_particles):
    ...
    for n in range_of_number_of_particles:  # range(8000,20000, 1000):
        ...
        t0 = time.time()
        hermite1.new_particle(values, values, values, values, values, values, values, values)
        t1 = time.time()
        d1 = t1 - t0
        #print d1, t1, t0
        ...
        t0 = time.time()
        for i in range(n - 1):
            hermite2.new_particle(values[i], values[i], values[i], values[i], values[i], values[i], values[i], values[i])
        t1 = time.time()
        d2 = t1 - t0
