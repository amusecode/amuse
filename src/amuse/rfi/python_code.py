...
class PythonImplementation(object):
    ...
    def start(self, mpi_port=None):
        ...
        while self.must_run:
            ...
            result_message = ClientSideMPIMessage(
                message.call_id, message.function_id, message.call_count
            )
            ...
            if message.function_id == 0:
                ...
                if message.function_id in self.mapping_from_tag_to_legacy_function:
                    ...
                    result_message.set_error(
                        "unknown function id " + str(message.function_id)
                    )
        ...
        while self.must_run:
            ...
            if message.function_id == 0:
                ...
                if message.function_id in self.mapping_from_tag_to_legacy_function:
                    ...
                    result_message.set_error(
                        "unknown function id " + message.function_id
                    )
    ...
    def handle_message(self, input_message, output_message):
        legacy_function = self.mapping_from_tag_to_legacy_function[
            input_message.function_id
        ]
        ...
        for type, attribute in self.dtype_to_message_attribute.items():
            ...
            for x in range(count):
                if type == "string":
                    getattr(output_message, attribute).append(
                        [""] * output_message.call_count
                    )
                else:
                    getattr(output_message, attribute).append(
                        numpy.zeros(output_message.call_count, dtype=type)
                    )
        ...
        if specification.must_handle_array:
            keyword_arguments = self.new_keyword_arguments_from_message(
                input_message, None, specification, input_units
            )
            try:
                result = method(**keyword_arguments)
            except TypeError as ex:
                warnings.warn(
                    "mismatch in python function specification(?): " + str(ex)
                )
                result = method(*list(keyword_arguments))
            self.fill_output_message(
                output_message, None, result, keyword_arguments, specification, units
            )
        else:
            for index in range(input_message.call_count):
                keyword_arguments = self.new_keyword_arguments_from_message(
                    input_message, index, specification, input_units
                )
                try:
                    result = method(**keyword_arguments)
                except TypeError as ex:
    ...
    def new_keyword_arguments_from_message(
        self, input_message, index, specification, units=[]
    ):
        ...
        for parameter in specification.parameters:
            ...
            if parameter.direction == LegacyFunctionSpecification.IN:
                if specification.must_handle_array:
                    argument_value = getattr(input_message, attribute)[
                ...
                if specification.must_handle_array:
                    argument_value = ValueHolder(
                        getattr(input_message, attribute)[parameter.input_index]
                    )
                else:
                    argument_value = ValueHolder(
                        getattr(input_message, attribute)[parameter.input_index][index]
                    )
    ...
    def fill_output_message(
        self, output_message, index, result, keyword_arguments, specification, units
    ):
        ...
        for parameter in specification.parameters:
            ...
            if (
                ...
                if specification.must_handle_array:
                    ...
                    getattr(output_message, attribute)[parameter.output_index][
                        index
                    ] = output
    ...
    def internal__accept_on_port(self, port_identifier, comm_identifier):
        ...
        rank = MPI.COMM_WORLD.Get_rank()
        if rank == 0:
            communicator = MPI.COMM_SELF.Accept(
                port_identifier, self.get_null_info(), 0
            )
    ...
    def internal__connect_to_port(self, port_identifier, comm_identifier):
        ...
        rank = MPI.COMM_WORLD.Get_rank()
        if rank == 0:
            communicator = MPI.COMM_SELF.Connect(
                port_identifier, self.get_null_info(), 0
            )
    ...
    def internal__become_code(self, number_of_workers, modulename, classname):
        ...
        key = (
            world.rank
            if world.rank < number_of_workers
