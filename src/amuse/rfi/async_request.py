...
class AbstractASyncRequest(object):
    ...
    def __rtruediv__(self, other):
        return baseOperatorASyncRequest(
            self, other, lambda x, y: operator.truediv(y, x)
        )
    ...
    def __rfloordiv__(self, other):
        return baseOperatorASyncRequest(
            self, other, lambda x, y: operator.floordiv(y, x)
        )
...
class baseOperatorASyncRequest(DependentASyncRequest):
    ...
    def result(self):
        ...
        second = (
            self._second.result()
            if isinstance(self._second, AbstractASyncRequest)
            else self._second
        )
        if second is None:
...
class ASyncRequestSequence(AbstractASyncRequest):
    ...
    def _next_request(self):
        if (
            self.current_async_request is not None
            and self.current_async_request.is_result_available()
        ):
...
class AsyncRequestsPool(object):
    ...
    def add_request(self, async_request, result_handler=None, args=(), kwargs={}):
        ...
        self.requests_and_handlers.append(
            AsyncRequestWithHandler(self, async_request, result_handler, args, kwargs)
        )
    ...
    def wait(self):
        ...
        while len(self.requests_and_handlers) > 0:
            requests = [
                x.async_request.waits_for()
                for x in self.requests_and_handlers
                if x.async_request.is_other()
            ]
            indices = [
                i
                for i, x in enumerate(self.requests_and_handlers)
                if x.async_request.is_other()
            ]
            if len(requests) > 0:
            ...
            requests_ = [
                x.async_request.waits_for().request
                for x in self.requests_and_handlers
            ...
            sockets_ = [
                x.async_request.waits_for().socket
                for x in self.requests_and_handlers
    ...
    def join(self, other):
        if other is None:
            ...
            for x in other.requests_and_handlers:
                self.add_request(
                    x.async_request, x.result_handler, args=x.args, kwargs=x.kwargs
                )
        else:
