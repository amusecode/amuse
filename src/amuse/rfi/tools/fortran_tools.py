...
class FortranCodeGenerator(object):
    ...
    def _grid_format_arg(self, name, dtype, ndim=1, index_ranges=None):
        ...
        if index_ranges is None:
            ...
            checks = (".OR. &\n" + " " * 11).join(
                [
                    "({0}.LT.{1}.OR.{0}.GT.{2})".format(
                        "i{0}(k)".format(i), index_ranges[i][0], index_ranges[i][1]
                    )
                    for i in range(ndim)
                ]
            )
    ...
    def grid_getter(self, name, dtype, ndim=1, index_ranges=None):
        return self._grid_getter_template.format(
            *self._grid_format_arg(name, dtype, ndim, index_ranges)
        )

    def grid_setter(self, name, dtype, ndim=1, index_ranges=None):
        return self._grid_setter_template.format(
            *self._grid_format_arg(name, dtype, ndim, index_ranges)
        )
    ...
    def parameter_getter_setters(self):
        ...
        for par, d in self.parameter_definition.items():
            if d["ptype"] in ["ro"]:
                filestring += self._getter_string.format(
                    d["short"], py_to_f[d["dtype"]]
                )
            if d["ptype"] in ["simple"]:
                filestring += self._setter_string.format(
                    d["short"], py_to_f[d["dtype"]]
                )
                filestring += self._getter_string.format(
                    d["short"], py_to_f[d["dtype"]]
            ...
            if d["ptype"] in ["vector"]:
                filestring += self._vector_setter_string.format(
                    d["short"], py_to_f[d["dtype"]]
                )
                filestring += self._vector_getter_string.format(
                    d["short"], py_to_f[d["dtype"]]
                )
            ...
            for pyvar, forvar in zip(d["pyvar"], d["forvar"]):
                if vartype != "ro":
                    output += (
                        ...
                        + "".join(
                            [
                                "  function.addParameter('index{0}', dtype='i', direction=function.IN)\n".format(
                                    i
                                )
                                for i in range(ndim)
                            ]
            ...
            if ptype in ["simple", "normal", "ro"]:
                if dtype != "bool":
                    object.add_method_parameter(
                        getter, setter, name, d["description"], d["default"]
                    )
                else:
                ...
                object.add_array_parameter(
                    getter, setter, range_method, name, d["description"]
                )
