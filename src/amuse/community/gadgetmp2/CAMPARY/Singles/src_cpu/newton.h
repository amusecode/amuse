...
static void inline invNewtonUp_aux(const float *x, float *res){
...
static void inline invNewtonUp(const float *x, float *res){
...
static void inline divNewton(const float *x, const float *y, float *res){
  float aux[sR];
...
template<int sX, int sR, int pDone>
static void inline invNewtonUp_fast_aux(const float *x, float *res){
  // pDone is the precision we have already computed in res
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  const float two[] = {2.};
  float aux[p];
...
template<int sX, int sR>
static void inline invNewtonUp_fast(const float *x, float *res){
  // starts by computing an approximation of the reciprocal, 1/x_0
  res[0] = 1./x[0];
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sY, int sR>
static void inline divNewton_fast(const float *x, const float *y, float *res){
  float aux[sR];
...
template<int sX, int sR, int pDone>
static void inline sqrtInvNewton_aux(const float *x, float *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  const float three[] = {3.};
  float aux[p];
...
template<int sX, int sR>
static void inline sqrtInvNewton(const float *x, float *res){
...
// SquareRoot
template<int sX, int sR>
static void inline sqrtNewton(const float *x, float *res){
	float aux[sR];
...
template<int sX, int sR, int pDone>
static void inline sqrtHeron_aux(const float *x, float *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  float aux[p];
...
template<int sX, int sR>
static void inline sqrtHeron(const float *x, float *res){
  if(x[0]<0.) for(int i=0; i<sR; i++) res[i] = nan("");
  else if(x[0]==0.) for(int i=0; i<sR; i++) res[i] = 0.;
  else{
    res[0] = sqrt(x[0]);
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sR, int pDone>
static void inline sqrtInvNewton_fast_aux(const float *x, float *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  const float three[] = {3.};
  float aux[p];
...
template<int sX, int sR>
static void inline sqrtInvNewton_fast(const float *x, float *res){
  if(x[0]<0.) for(int i=0; i<sR; i++) res[i] = nan("");
  else if(x[0]==0.) for(int i=0; i<sR; i++) res[i] = 0.;
  else{
    res[0] = 1./sqrt(x[0]);
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sR>
static void inline sqrtNewton_fast(const float *x, float *res){
	float aux[sR];
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sR, int pDone>
static void inline sqrtHeron_fast_aux(const float *x, float *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  float aux[p];
...
template<int sX, int sR>
static void inline sqrtHeron_fast(const float *x, float *res){
  if(x[0]<0.) for(int i=0; i<sR; i++) res[i] = nan("");
  else if(x[0]==0.) for(int i=0; i<sR; i++) res[i] = 0.;
  else{
    res[0] = sqrt(x[0]);
