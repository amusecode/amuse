...
static inline void merge(float const *x, float const *y, float *z){
  int i=0, j=0;
  for(int n=0; n<K+L; n++){
...
static inline void certifiedAdd(const float *x, const float *y, float *r){
...
template <int K, int L, int R>
static inline void baileyAdd_fast(float const *x, float const *y, float *z){
...
template <int K, int L, int R>
static inline void baileyAdd_renorm(float const *x, float const *y, float *z){
  if(L == 1) renorm2L_4Add1<K,R>(x, y[0], z);
  else{
	  int i, n;
	  float e, f[R+1];
