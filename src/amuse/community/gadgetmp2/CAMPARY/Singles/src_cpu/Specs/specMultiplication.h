...
/****************************************************/
template<> inline void certifiedMul<1,1,1>(float const *x, float const *y, float *r){ r[0] = FPmul_rn(x[0], y[0]); }
template<> inline void certifiedMul<1,1,2>(float const *x, float const *y, float *r){ r[0] = two_prod(x[0], y[0], r[1]); }
template<> inline void certifiedMul<1,2,2>(float const *x, float const *y, float *r){
...
template<> 
inline void certifiedMul<2,1,2>(float const *x, float const *y, float *r){
// DWTimesFP1 or DWTimesFP3 with relative error <= 2u^2
  float ch, cl1, cl2;
  ch = two_prod( x[0], y[0], cl1 );  
  #if defined __CUDA_ARCH__ || FP_FAST_FMA
...
template<> 
inline void certifiedMul<2,2,2>(float const *x, float const *y, float *r){
...
/****************************************************/
template<> inline void truncatedMul<1,1,1>(float const *x, float const *y, float *z){ z[0] = FPmul_rn(x[0], y[0]); }
template<> inline void truncatedMul<1,1,2>(float const *x, float const *y, float *z){ z[0] = two_prod(x[0], y[0], z[1]); }
template<>
inline void truncatedMul<2,1,2>(float const *x, float const *y, float *z){
...
template<>
inline void truncatedMul<2,2,2>(float const *x, float const *y, float *z){
...
template<>
inline void truncatedMul<3,1,3>(float const *x, float const *y, float *z){
  float e, f[4];
...
template<>
inline void truncatedMul<3,3,3>(float const *x, float const *y, float *z){
  float p, e, f[4];
...
template<>
inline void truncatedMul<4,1,4>(float const *x, float const *y, float *z){
  float e, f[5];
...
template<>
inline void truncatedMul<4,4,4>(float const *x, float const *y, float *z){
  float p, e, f[5];
...
template<>
inline void truncatedMul<3,2,2>(float const *x, float const *y, float *z){
  float p, e, f[3];
...
template<>
inline void truncatedMul<3,2,3>(float const *x, float const *y, float *z){
  float p, e, f[4];
...
template<>
inline void truncatedMul<4,2,2>(float const *x, float const *y, float *z){
  float p, e, f[3];
  f[2] = FPmul_rn( x[1], y[1] );
  #if defined __CUDA_ARCH__ || FP_FAST_FMA
    f[2] = FPfma_rn( x[2], y[0], f[2] );
...
template<>
inline void truncatedMul<4,2,4>(float const *x, float const *y, float *z){
  float p, e, f[5];
...
/****************************************************/
template<> inline void baileyMul_fast<1,1,1>(float const *x, float const *y, float *z){ z[0] = FPmul_rn(x[0], y[0]); }
template<> inline void baileyMul_fast<1,1,2>(float const *x, float const *y, float *z){ z[0] = two_prod(x[0], y[0], z[1]); }
template<>
inline void baileyMul_fast<2,1,2>(float const *x, float const *y, float *z){
// DWTimesFP2 with relative error <= 3u^2 or DWTimesFP3 with relative error <= 2u^2
  float ch, cl1, cl2;
  ch = two_prod( x[0], y[0], cl1 );
  #if defined __CUDA_ARCH__ || FP_FAST_FMA
...
template<>
inline void baileyMul_fast<2,2,2>(float const *x, float const *y, float *z){
// DWTimesDW1 with relative error <= 7u^2 or DWTimesDW2 with relative error <= 6u^2
  float ch, cl, tl1, tl2;
  ch = two_prod( x[0], y[0], cl );
  #if defined __CUDA_ARCH__ || FP_FAST_FMA
...
template<>
inline void baileyMul_fast<3,1,3>(float const *x, float const *y, float *z){
  float e, f[4];
  f[2] = two_prod( x[2], y[0], f[3] );
  f[1] = two_prod( x[1], y[0], e );
  f[2] = two_sum ( f[2], e,    e );
...
template<>
inline void baileyMul_fast<3,3,3>(float const *x, float const *y, float *z){
  float p, e, f[4];
  f[3] = FPmul_rn( x[1], y[2] );
  #if defined __CUDA_ARCH__ || FP_FAST_FMA
    f[3] = FPfma_rn( x[2], y[1], f[3] );
...
template<>
inline void baileyMul_fast<4,1,4>(float const *x, float const *y, float *z){
  float e, f[5];
  f[3] = two_prod( x[3], y[0], f[4] );
  f[2] = two_prod( x[2], y[0], e );
  f[3] = two_sum ( f[3], e,    e ); 
...
template<>
inline void baileyMul_fast<4,4,4>(float const *x, float const *y, float *z){
  float p, e, f[5];
  f[4] = FPmul_rn( x[1], y[3] );
  #if defined __CUDA_ARCH__ || FP_FAST_FMA
    f[4] = FPfma_rn( x[2], y[2], f[4] );
...
template<>
inline void baileyMul_fast<5,1,5>(float const *x, float const *y, float *z){
	float e, f[6];
	f[5] = 0.0;
...
template<>
inline void baileyMul_fast<5,5,5>(float const *x, float const *y, float *z){
	float p, e, f[6];
...
template<>
inline void baileyMul_fast<6,1,6>(float const *x, float const *y, float *z){
	float e, f[7];
	f[6] = 0.0;
...
template<>
inline void baileyMul_fast<6,6,6>(float const *x, float const *y, float *z){
	float p, e, f[7];
...
template<>
inline void baileyMul_fast<7,1,7>(float const *x, float const *y, float *z){
	float e, f[8];
	f[7] = 0.0;
...
template<>
inline void baileyMul_fast<7,7,7>(float const *x, float const *y, float *z){
	float p, e, f[8];
...
template<>
inline void baileyMul_fast<8,1,8>(float const *x, float const *y, float *z){
	float e, f[9];
	f[8] = 0.0;
...
template<>
inline void baileyMul_fast<8,8,8>(float const *x, float const *y, float *z){
	float p, e, f[9];
...
template<>
inline void baileyMul_fast<3,2,2>(float const *x, float const *y, float *z){
  float p, e, f[3];
  f[2] = FPmul_rn( x[1], y[1] );
  #if defined __CUDA_ARCH__ || FP_FAST_FMA
    f[2] = FPfma_rn( x[2], y[0], f[2] );
...
template<>
inline void baileyMul_fast<3,2,3>(float const *x, float const *y, float *z){
  float p, e, f[4];
  f[3] = FPmul_rn( x[2], y[1] ); 
...
template<>
inline void baileyMul_fast<4,2,2>(float const *x, float const *y, float *z){
  float p, e, f[3];
  f[2] = FPmul_rn( x[1], y[1] );
  #if defined __CUDA_ARCH__ || FP_FAST_FMA
    f[2] = FPfma_rn( x[2], y[0], f[2] );
...
template<>
inline void baileyMul_fast<4,2,4>(float const *x, float const *y, float *z){
  float p, e, f[5];
  f[4] = FPmul_rn( x[3], y[1] );
  f[3] = two_prod( x[3], y[0], e );
  f[4] = FPadd_rn( f[4], e );
...
/****************************************************/
template<> inline void baileyMul_renorm<1,1,1>(float const *x, float const *y, float *z){ z[0] = FPmul_rn(x[0], y[0]); }
template<> inline void baileyMul_renorm<1,1,2>(float const *x, float const *y, float *z){ z[0] = two_prod(x[0], y[0], z[1]); }
template<> 
inline void baileyMul_renorm<2,1,2>(float const *x, float const *y, float *z){
// DWTimesFP1 or DWTimesFP3 with relative error <= 2u^2
  float ch, cl1, cl2;
  ch = two_prod( x[0], y[0], cl1 );
  #if defined __CUDA_ARCH__ || FP_FAST_FMA
...
template<> 
inline void baileyMul_renorm<2,2,2>(float const *x, float const *y, float *z){
// DWTimesDW1 with relative error <= 7u^2 or DWTimesDW2 with relative error <= 6u^2
  float ch, cl, tl1, tl2;
  ch = two_prod( x[0], y[0], cl );
  #if defined __CUDA_ARCH__ || FP_FAST_FMA
