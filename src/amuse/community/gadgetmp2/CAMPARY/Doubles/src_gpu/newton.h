...
/***************************************************************
 ----------------- DIVISION ALGORITHMS -------------------
 ***************************************************************/
/** Computes the reciprocal of sX doubles and returns the result on an sR double ulp-nonoveralpping expansion.
    sX - size of x, sR - size of res.
    Uses a recursive Newton-Raphson iteration method for 1/y, with initial guess q_0; 
    The iteration is r_{n+1} = r_n(2-xr_n).
...
__host__ __device__ static void __forceinline__ invNewtonUp_aux(const double *x, double *res){
...
__host__ __device__ static void __forceinline__ invNewtonUp(const double *x, double *res){
  // starts by computing an approximation of the reciprocal, 1/x_0
  res[0] = 1./x[0];
  if(sR > 1) invNewtonUp_aux<sX,sR,1>(x, res);
}

/** Computes the division of sX doubles by sY doubles and returns the result on an sR double ulp-nonoveralpping expansion.
...
__host__ __device__ static void __forceinline__ divNewton(const double *x, const double *y, double *res){
  double aux[sR];
...
template<int sX, int sR, int pDone>
__host__ __device__ static void __forceinline__ invNewtonUp_fast_aux(const double *x, double *res){
  // pDone is the precision we have already computed in res
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  const double two[] = {2.};
  double aux[p];
...
template<int sX, int sR>
__host__ __device__ static void __forceinline__ invNewtonUp_fast(const double *x, double *res){
  // starts by computing an approximation of the reciprocal, 1/x_0
  res[0] = 1./x[0];
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sY, int sR>
__host__ __device__ static void __forceinline__ divNewton_fast(const double *x, const double *y, double *res){
  double aux[sR];
 ...
 ***************************************************************/
/** Computes the reciprocal of the square root of sX doubles and returns the result on an sR double ulp-nonoveralpping expansion.
    sX - size of x, sR - size of res.
...
template<int sX, int sR, int pDone>
__host__ __device__ static void __forceinline__ sqrtInvNewton_aux(const double *x, double *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
...
template<int sX, int sR>
__host__ __device__ static void __forceinline__ sqrtInvNewton(const double *x, double *res){
...
/** Computes the square root of sX doubles and returns the result on an sR double ulp-nonoveralpping expansion.
    sX - size of x, sR - size of res.
    Firstly computes the reciprocal of sqrt(x) and after that multiplies the result by x **/
// SquareRoot
template<int sX, int sR>
__host__ __device__ static void __forceinline__ sqrtNewton(const double *x, double *res){
	double aux[sR];
...
template<int sX, int sR, int pDone>
__host__ __device__ static void __forceinline__ sqrtHeron_aux(const double *x, double *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  double aux[p];
...
template<int sX, int sR>
__host__ __device__ static void __forceinline__ sqrtHeron(const double *x, double *res){
  if(x[0]<0.) for(int i=0; i<sR; i++) res[i] = nan("");
  else if(x[0]==0.) for(int i=0; i<sR; i++) res[i] = 0.;
  else{
    res[0] = sqrt(x[0]);
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sR, int pDone>
__host__ __device__ static void __forceinline__ sqrtInvNewton_fast_aux(const double *x, double *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  const double three[] = {3.};
  double aux[p];
...
template<int sX, int sR>
__host__ __device__ static void __forceinline__ sqrtInvNewton_fast(const double *x, double *res){
  if(x[0]<0.) for(int i=0; i<sR; i++) res[i] = nan("");
  else if(x[0]==0.) for(int i=0; i<sR; i++) res[i] = 0.;
  else{
    res[0] = 1./sqrt(x[0]);
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sR>
__host__ __device__ static void __forceinline__ sqrtNewton_fast(const double *x, double *res){
	double aux[sR];
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sR, int pDone>
__host__ __device__ static void __forceinline__ sqrtHeron_fast_aux(const double *x, double *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  double aux[p];
...
template<int sX, int sR>
__host__ __device__ static void __forceinline__ sqrtHeron_fast(const double *x, double *res){
  if(x[0]<0.) for(int i=0; i<sR; i++) res[i] = nan("");
  else if(x[0]==0.) for(int i=0; i<sR; i++) res[i] = 0.;
  else{
    res[0] = sqrt(x[0]);
