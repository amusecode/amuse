...
/*****************************************************************************************/
__host__ __device__ static __forceinline__ double FPadd_rn(const double x, const double y){
...
/**********************************************************************************************/
__host__ __device__ static __forceinline__ double FPmul_rn(const double x, const double y){
  #if defined __CUDA_ARCH__
...
/**********************************************************************************************/
__host__ __device__ static __forceinline__ double FPdiv_rn(const double x, const double y){
  #if defined __CUDA_ARCH__
...
/*************************************************************************************************/
/*Dekker's algorithm
  @ requires xy == \round_double(\NearestEven,x*y) && \abs(x) <= 0x1.p995 && 
  @          \abs(y) <= 0x1.p995 && \abs(x*y) <=  0x1.p1021;
  @ ensures  ((x*y == 0 || 0x1.p-969 <= \abs(x*y)) ==> x*y == xy+\result); @*/
__host__ static __forceinline__ double fma_d_rn_cpu(const double x, const double y, double xy) {
...
__host__ __device__ static __forceinline__ double FPfma_rn(const double x, const double y, const double z){
  #if defined __CUDA_ARCH__
