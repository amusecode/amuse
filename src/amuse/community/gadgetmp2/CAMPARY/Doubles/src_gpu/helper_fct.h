...
/***************************************************************/
template<int size>
__host__ __device__ static void printArray(const double *p){
	printf("Array size %d,\n ",size);
...
template<int size>
__host__ __device__ static void printArrayBin(const double *p){
	printf("Array size %d,\n ",size);
...
/***************************************************/
template<>
__device__ double unifRand_gpu<double>(){
 	curandState s;
...
/***************************************************************/
__host__ __device__ static int ulpExp(double x){
	int e;
...
template<int size>
__host__ __device__ static void genExpans_P(double *res){
  res[0] = unifRand<double>(-1.0, 1.0);
  int exp0 = 1;
  res[0] = ldexp(res[0], exp0);
  for(int i=1; i<size; i++){
  	res[i] = unifRand<double>(-1.0, 1.0);
  	exp0 = unifRand<int>(1,5);
  	res[i] = ldexp(res[i], ulpExp(res[i-1]) - exp0);
  }
}
template<> __host__ __device__ void genExpans_P<2>(double *res){
  res[0] = unifRand<double>(-1.0, 1.0);
  int exp0 = 1;
  res[0] = ldexp(res[0], exp0);
 	res[1] = unifRand<double>(-1.0, 1.0);
  exp0 = unifRand<int>(1,5);
  res[1] = ldexp(res[1], ulpExp(res[0]) - exp0);
  double s = res[0] + res[1];
  double z = s -res[0];
  res[0] = s;
  res[1] = res[1] -z;
...
template<int size>
__host__ __device__ static void genExpans_ulp(double *res){
	res[0] = unifRand<double>(-1.0, 1.0);
...
template<> __host__ __device__ void genExpans_ulp<2>(double *res){
  res[0] = unifRand<double>(-1.0, 1.0);
  int exp0 = 1;
  res[0] = ldexp(res[0], exp0);
 	res[1] = unifRand<double>(-1.0, 1.0);
...
template<int size>
__host__ __device__ static void genExpans_ulp(double *res, int order){
  res[0] = unifRand<double>(0.5, 1.0);
...
template<> __host__ __device__ void genExpans_ulp<2>(double *res, int order){
  res[0] = unifRand<double>(0.5, 1.0);
  res[0] = ldexp(res[0], order);
  int eq, exp0;
  eq = unifRand<int>(0,100) % 2;
...
template<int size>
__host__ __device__ static void genExpans_ulp(double *res, double first){
  res[0] = first;
...
template<> __host__ __device__ void genExpans_ulp<2>(double *res, double first){
  res[0] = first;
  int eq, exp0;
  eq = unifRand<int>(0,100) % 2;
  if(eq == 0){
...
template<int size>
__host__ __device__ static void genOverlappingExpans(double *res){
  res[0] = unifRand<double>(-1.0,1.0);
  int exp0 = 1;
  res[0] = ldexp(res[0],exp0);
  for(int i=1; i<size; i++){
