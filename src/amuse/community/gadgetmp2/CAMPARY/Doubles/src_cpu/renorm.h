...
struct unrollVecSum{
  static inline void fast_VecSum(double *x, double s){
  ...
  static inline void VecSum(double *x, double s){
...
struct unrollVecSum<from,from,step>{
  static inline void fast_VecSum(double *x, double s){ x[0] = s; }
  static inline void fast_VecSum(const double *x, double s, double *r){ r[0] = s; }
  static inline void VecSum(double *x, double s){ x[0] = s; }
  static inline void VecSum(const double *x, double s, double *r){ r[0] = s; }
};

template<int sX>
static inline void fast_VecSum(double *x){
...
template<int sX>
static inline void fast_VecSum(const double *x, double *r){
...
template<int sX>
static inline void VecSum(double *x){
  double s = fast_two_sum(x[sX-2], x[sX-1], x[sX-1]);
  #ifdef UNROLL_RENORM
...
static inline void fast_VecSumErrBranch(double *x){
	int ptr = 0, i = 1;
	double e = x[0];
...
/***************************************************************/
template<int sX>
static inline void fast_VecSumErr(double *x){
	double e;
...
template<int sX>
static inline void fast_VecSumErr(const double *x, double *r){
  double e;
...
// Renormalize
template<int sX, int sR>
static inline void fast_renorm2L(double *x){
	double e;
	int ptr = 0, i = 2;
...
template<int sX, int sR>
static inline void fast_renorm2L(const double *x, double *r){
  double e, f[sX];
  int ptr = 0, i = 2;
...
template<int sX, int sR>
static inline void renorm2L(double *x){
  double e;
  int ptr = 0, i = 2;
...
/** Algorithm for normalizing the array x. The result satisfies |x_i|<ulp(x_{i-1}); P-nonoverlapping expansion.
    ...
    Requirement: sR <= sX **/
template<int sX, int sR>
static inline void fast_renorm3L(double *x){
  double e;
	int ptr = 0, i = 2;
...
template<int sX, int sR>
static inline void fast_renorm3L(double *x, double *r){
  double e, f[sX];
  int ptr = 0, i = 2;
...
// Renormalize_random
template<int sX, int sR>
static inline void renorm_rand2L(double *x){
	double pr;
	int i, j, ptr = 0;
