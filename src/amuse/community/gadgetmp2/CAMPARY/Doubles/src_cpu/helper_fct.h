...
/***************************************************************/
template<int size>
static void printArray(const double *p){
	printf("Array size %d,\n ",size);
...
template<int size>
static void printArrayBin(const double *p){
	printf("Array size %d,\n ",size);
...
template<int size>
static void genExpans_P(double *res){
...
template<> void genExpans_P<2>(double *res){
  res[0] = unifRand<double>(-1.0, 1.0);
  int exp0 = 1;
  res[0] = ldexp(res[0], exp0);
...
template<int size>
static void genExpans_ulp(double *res){
	res[0] = unifRand<double>(-1.0, 1.0);
...
template<> void genExpans_ulp<2>(double *res){
  res[0] = unifRand<double>(-1.0, 1.0);
  int exp0 = 1;
  res[0] = ldexp(res[0], exp0);
 	res[1] = unifRand<double>(-1.0, 1.0);
...
template<int size>
static void genExpans_ulp(double *res, int order){
...
template<> void genExpans_ulp<2>(double *res, int order){
  res[0] = unifRand<double>(0.5, 1.0);
  res[0] = ldexp(res[0], order);
  int eq, exp0;
  eq = unifRand<int>(0,100) % 2;
...
template<int size>
static void genExpans_ulp(double *res, double first){
  res[0] = first;
...
template<> void genExpans_ulp<2>(double *res, double first){
  res[0] = first;
  int eq, exp0;
  eq = unifRand<int>(0,100) % 2;
  if(eq == 0){
...
template<int size>
static void genOverlappingExpans(double *res){
  res[0] = unifRand<double>(-1.0,1.0);
  int exp0 = 1;
  res[0] = ldexp(res[0],exp0);
  for(int i=1; i<size; i++){
