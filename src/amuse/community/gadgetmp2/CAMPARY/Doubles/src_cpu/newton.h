...
static void inline invNewtonUp_aux(const double *x, double *res){
...
static void inline invNewtonUp(const double *x, double *res){
...
static void inline divNewton(const double *x, const double *y, double *res){
  double aux[sR];
...
template<int sX, int sR, int pDone>
static void inline invNewtonUp_fast_aux(const double *x, double *res){
  // pDone is the precision we have already computed in res
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  const double two[] = {2.};
  double aux[p];
...
template<int sX, int sR>
static void inline invNewtonUp_fast(const double *x, double *res){
  // starts by computing an approximation of the reciprocal, 1/x_0
  res[0] = 1./x[0];
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sY, int sR>
static void inline divNewton_fast(const double *x, const double *y, double *res){
  double aux[sR];
...
template<int sX, int sR, int pDone>
static void inline sqrtInvNewton_aux(const double *x, double *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
...
template<int sX, int sR>
static void inline sqrtInvNewton(const double *x, double *res){
...
// SquareRoot
template<int sX, int sR>
static void inline sqrtNewton(const double *x, double *res){
	double aux[sR];
...
template<int sX, int sR, int pDone>
static void inline sqrtHeron_aux(const double *x, double *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  double aux[p];
...
template<int sX, int sR>
static void inline sqrtHeron(const double *x, double *res){
  if(x[0]<0.) for(int i=0; i<sR; i++) res[i] = nan("");
  else if(x[0]==0.) for(int i=0; i<sR; i++) res[i] = 0.;
  else{
    res[0] = sqrt(x[0]);
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sR, int pDone>
static void inline sqrtInvNewton_fast_aux(const double *x, double *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  const double three[] = {3.};
  double aux[p];
...
template<int sX, int sR>
static void inline sqrtInvNewton_fast(const double *x, double *res){
  if(x[0]<0.) for(int i=0; i<sR; i++) res[i] = nan("");
  else if(x[0]==0.) for(int i=0; i<sR; i++) res[i] = 0.;
  else{
    res[0] = 1./sqrt(x[0]);
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sR>
static void inline sqrtNewton_fast(const double *x, double *res){
	double aux[sR];
...
// quick and dirty method that doesn't guaranty the error bound and that the intermediate expansions are ulp-nonoverlapping
template<int sX, int sR, int pDone>
static void inline sqrtHeron_fast_aux(const double *x, double *res){
  const int p  = (2*pDone>sR) ? sR : 2*pDone;
  double aux[p];
...
template<int sX, int sR>
static void inline sqrtHeron_fast(const double *x, double *res){
  if(x[0]<0.) for(int i=0; i<sR; i++) res[i] = nan("");
  else if(x[0]==0.) for(int i=0; i<sR; i++) res[i] = 0.;
  else{
    res[0] = sqrt(x[0]);
