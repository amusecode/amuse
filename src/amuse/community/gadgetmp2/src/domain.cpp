 ...
 *  as a tuning measure.
 */
void gadgetmp2::domain_Decomposition(void)
{
    double t0, t1;
    ...
    if(All.NumForcesSinceLastDomainDecomp > All.TotNumPart * All.TreeDomainUpdateFrequency)
    {
        ...
        Key = new peanokey[All.MaxPart];
        KeySorted = new peanokey[All.MaxPart];
        toGo =  new int[NTask * NTask];
        toGoSph =  new int[NTask * NTask];
        local_toGo = new int[NTask];
        local_toGoSph = new int[NTask];
        list_NumPart = new int[NTask];
        list_N_gas = new int[NTask];
        list_load = new int[NTask];
 ...
 *  it was assigned to.
 */
int gadgetmp2::domain_findSplit(int cpustart, int ncpu, int first, int last)
{
    int i, split, ok_left, ok_right;
    long long load, sphload, load_leftOfSplit, sphload_leftOfSplit;
    int ncpu_leftOfSplit;
    my_float maxAvgLoad_CurrentSplit, maxAvgLoad_NewSplit;
 ...
 */
void gadgetmp2::domain_shiftSplit(void)
{
 ...
 */
void gadgetmp2::domain_findExchangeNumbers(int task, int partner, int sphflag, int *send, int *recv)
{
...
void gadgetmp2::domain_exchangeParticles(int partner, int sphflag, int send_count, int recv_count)
{
    ...
    for(n = 0, count = 0; count < send_count && n < NumPart; n++)
    {
        ...
        if(DomainTask[no] == partner)
        {
            if(sphflag)		/* special reorder routine for SPH particles (need to stay at beginning) */
            {
                ...
                DomainPartBuf_s->l_fill(P[n], count);
                DomainKeyBuf[count] = Key[n];
                //DomainSphBuf[count] = SphP[n];
                DomainSphBuf_s->l_fill(SphP[n], count);
    ...
    for(rep = 0; rep < 2; rep++)
    {
        if((rep == 0 && ThisTask < partner) || (rep == 1 && ThisTask > partner))
        {
            if(send_count > 0)
            {
                MPI_Ssend(DomainPartBuf_s, send_count * DomainPartBuf_s->get_size(), MPI_BYTE, partner, TAG_PDATA, GADGET_WORLD);

                MPI_Ssend(&DomainKeyBuf[0], send_count * sizeof(peanokey), MPI_BYTE, partner, TAG_KEY, GADGET_WORLD);

                if(sphflag)
                    MPI_Ssend(DomainSphBuf_s, send_count * DomainSphBuf_s->get_size(), MPI_BYTE, partner, TAG_SPHDATA, GADGET_WORLD);
            }
        }
        ...
        if((rep == 1 && ThisTask < partner) || (rep == 0 && ThisTask > partner))
        {
            if(recv_count > 0)
            {
                if(sphflag)
                {
                    ...
                    //MPI_Recv(&P[N_gas], recv_count * sizeof(struct particle_data), MPI_BYTE, partner, TAG_PDATA, GADGET_WORLD, &status);
                    MPI_Recv(DomainPartBuf_r, recv_count * DomainPartBuf_r->get_size(), MPI_BYTE, partner, TAG_PDATA,
                             GADGET_WORLD, &status);
                    MPI_Recv(&Key[N_gas], recv_count * sizeof(peanokey), MPI_BYTE, partner, TAG_KEY,
                             GADGET_WORLD, &status);
                    //MPI_Recv(&SphP[N_gas], recv_count * sizeof(struct sph_particle_data), MPI_BYTE, partner, TAG_SPHDATA, GADGET_WORLD, &status);
                    MPI_Recv(DomainSphBuf_r, recv_count * DomainSphBuf_r->get_size(), MPI_BYTE, partner,
                             TAG_SPHDATA, GADGET_WORLD, &status);
                    for (b=0, c=N_gas;b<recv_count; b++)
                    {
                        P[c]=DomainPartBuf_r->unpack_particle(b);
                        SphP[c]=DomainSphBuf_r->unpack_particle(b);
                        c++;
                    }
                    N_gas += recv_count;
                }
                ...
                {
                    //MPI_Recv(&P[NumPart], recv_count * sizeof(struct particle_data), MPI_BYTE, partner, TAG_PDATA, GADGET_WORLD, &status);
                    MPI_Recv(DomainPartBuf_r, recv_count * DomainPartBuf_r->get_size(), MPI_BYTE, partner,
                             TAG_PDATA, GADGET_WORLD, &status);
                    MPI_Recv(&Key[NumPart], recv_count * sizeof(peanokey), MPI_BYTE, partner,
                             TAG_KEY, GADGET_WORLD, &status);
                    for (b=0, c=NumPart;b<recv_count; b++)
                    {
                        P[c]=DomainPartBuf_r->unpack_particle(b);
                        c++;
                    }
 ...
 *  decomposition.
 */
void gadgetmp2::domain_countToGo(void)
{
    int n, no;
 ...
 *  processors.
 */
void gadgetmp2::domain_walktoptree(int no)
{
    int i;
 ...
 */
void gadgetmp2::domain_sumCost(void)
{
    int i, n, no;
    my_float *local_DomainWork;
    int *local_DomainCount;
    int *local_DomainCountSph;

    local_DomainWork = new my_float[NTopnodes];
    local_DomainCount = new int[NTopnodes];
    local_DomainCountSph = new int[NTopnodes];
    ...
    delete[] local_DomainWork;
}


/*! This routine finds the extent of the global domain grid.
 */
void gadgetmp2::domain_findExtent(void)
{
    int i, j;
    my_float len, xmin[3], xmax[3], xmin_glob[3], xmax_glob[3];
 ...
 *  number of particles.
 */
void gadgetmp2::domain_determineTopTree(void)
{
    ...
    toplist_local = new topnode_exchange[NTopnodes];
    ...
    toplist =  new topnode_exchange[ntop];
 ...
 */
void gadgetmp2::domain_topsplit_local(int node, peanokey startkey)
{
    int i, p, sub, bin;
 ...
 *  (TOPNODEFACTOR * NTask).
 */
void gadgetmp2::domain_topsplit(int node, peanokey startkey)
{
    int i, p, sub, bin;
 ...
 */
int gadgetmp2::domain_compare_toplist(const void *a, const void *b)
{
...
/*! This is a comparison kernel used in a sort routine.
 */
int gadgetmp2::domain_compare_key(const void *a, const void *b)
{
