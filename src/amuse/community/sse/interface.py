"""
AMUSE interface for the stellar population synthesis code SSE
"""
...
class SseInterface(
    ...
    def evolve_star():
        ...
        function.addParameter(
            "kw", dtype="i", direction=function.INOUT, unit=units.stellar_type
        )
        function.addParameter(
            "mass", dtype="d", direction=function.INOUT, unit=units.MSun
        )
        function.addParameter(
            "mt", dtype="d", direction=function.INOUT, unit=units.MSun
        )
        function.addParameter("r", dtype="d", direction=function.INOUT, unit=units.RSun)
        function.addParameter(
            "lum", dtype="d", direction=function.INOUT, unit=units.LSun
        )
        function.addParameter(
            "mc", dtype="d", direction=function.INOUT, unit=units.MSun
        )
        function.addParameter(
            "rc", dtype="d", direction=function.INOUT, unit=units.RSun
        )
        function.addParameter(
            "menv", dtype="d", direction=function.INOUT, unit=units.MSun
        )
        function.addParameter(
            "renv", dtype="d", direction=function.INOUT, unit=units.RSun
        )
        function.addParameter(
            "ospin", dtype="d", direction=function.INOUT, unit=units.yr**-1
        )
        function.addParameter(
            "epoch", dtype="d", direction=function.INOUT, unit=units.Myr
        )
        function.addParameter("tm", dtype="d", direction=function.INOUT, unit=units.Myr)
        function.addParameter(
            "tphys", dtype="d", direction=function.INOUT, unit=units.Myr
        )
        function.addParameter(
            "tphysf", dtype="d", direction=function.INOUT, unit=units.Myr
        )
        return function
    ...
    def get_time_step():
        ...
        function.can_handle_array = True
        function.addParameter(
            "kw", dtype="i", direction=function.IN, unit=units.stellar_type
        )
    ...
    def get_mass_loss_wind():
        ...
        function.addParameter(
            "mlout", dtype="d", direction=function.OUT, unit=units.MSun / units.yr
        )
...
class SseParticles(Particles):
    ...
    def add_particles_to_store(self, keys, attributes=[], values=[]):
        ...
        super(SseParticles, self).add_particles_to_store(
...
class Sse(common.CommonCode):
    ...
    def define_parameters(self, handler):
        ...
        handler.add_caching_parameter(
            ...
            "ifflag > 0 uses white dwarf IFMR (initial-final mass relation) of HPE, "
            "1995, MNRAS, 272, 800 (0).",
            0,
        )
        ...
        handler.add_caching_parameter(
            ...
            "nsflag > 0 takes NS/BH mass from Belczynski et al. 2002, ApJ, 572, "
            "407 (1).",
            1,
        )
        ...
        handler.add_caching_parameter(
            ...
            "The timesteps chosen in each evolution phase as decimal fractions of the "
            "time taken in that phase: MS (0.05)",
            0.05,
        )
        ...
        handler.add_caching_parameter(
            ...
            "The timesteps chosen in each evolution phase as decimal fractions of the "
            "time taken in that phase: GB, CHeB, AGB, HeGB (0.01)",
            0.01,
        )
        ...
        handler.add_caching_parameter(
            ...
            "The timesteps chosen in each evolution phase as decimal fractions of the "
            "time taken in that phase: HG, HeMS (0.02)",
            0.02,
        )
    ...
    def _evolve_particles(self, particles, end_time):
        ...
        star_type = result[0].value_in(units.stellar_type)
        helium_star_selection = (star_type > 6) & (star_type < 16) & (star_type != 10)
        ...
        if len(helium_stars):
            ...
            helium_stars.set_values_in_store(
                helium_stars.get_all_indices_in_store(),
                (
                    "stellar_type",
                    "initial_mass",
                    "mass",
                ...
                helium_stars_results,
            )
        if len(other_stars):
            other_star_selection = numpy.logical_not(helium_star_selection)
            other_stars.set_values_in_store(
                other_stars.get_all_indices_in_store(),
                attributes,
    ...
    def evolve_model(self, end_time=None, keep_synchronous=True):
        if not keep_synchronous:
            self._evolve_particles(
                self.particles, self.particles.time_step + self.particles.age
            )
            return
        ...
        self._evolve_particles(
            self.particles, end_time - self.model_time + self.particles.age
        )
        self.model_time = end_time

    def _evolve_model_old(self, end_time=None, keep_synchronous=True):
        """
        This is the old implementation of evolve_model. Even with
        (keep_synchronous = True) it is unable to evolve all stars to a common
        age, since it relies on the individual timesteps as determined by the
        community code. Furthermore, it is not suited to simulations with
        ongoing star formation, since it evolves newly created stars to the
        same age as the old stars.
        """
        if end_time is None:
            if keep_synchronous:
