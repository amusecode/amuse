...
public:
  ...
  template<typename T> void write_data(const char *dataset_path, int *offset, arr_1D<T>* data);
 ...
 */
template<typename T> void h5w::write_data(const char *dataset_path, int *offset, arr_1D<T>* data)
{
/*
...
*/
  ...
  if (rank < data->get_rank())
    {
      cout << "Rank of dataset smaller than rank of input data for dataset: " << dataset_path << endl;
      cout << "Exiting!" << endl;
      exit(-1);
    }
  ...
  for (i=0; i<rank; i++)
    if ( (int) dim[i] < data->get_dims()[i])
      {
        cout << "Size of input data chunk larger than the size of dataset along one or more dimensions" << endl;
        cout << "Exiting!" << endl;
        exit(-1);
      }
...
/*
  Write atribute associated to a specified dataset of group
*/
...
// write_arr overload to write an array of values from arr_1D<T> instance reference
template<typename T> void h5w::write_attr(const char *dataset_name, const char *attr_name, arr_1D<T> *data)
{
  int i;
  herr_t status;
  hid_t group, dataset, object; // first 2 are temporary vars to see if the object is a group or dataset
  ...
  attr_dims = new hsize_t [data->get_rank()];
  for (i=0; i< data->get_rank(); i++)
    attr_dims[i] = data->get_dims()[i];
  
  attr_dataspace = H5Screate_simple(data->get_rank(), attr_dims, NULL);
  // create new attribute
  attr = H5Acreate(object, attr_name, datatype_in, attr_dataspace, H5P_DEFAULT);
  //write 
  status = H5Awrite(attr, datatype_in, data->get_arr());
  if (status < 0)
    {
      cout<< "Writing atribute: " << attr_name << " to object: " << dataset_name << " failed !" << endl << "Exiting!" << endl;
      exit(-1);
