...
public:
  ...
  const float& get_n_HII() const{ return n_HII; }
    ...
    //! assign temperature to site
  void set_temperature(const float& _T){ temperature = _T; }
...
private:
  ...
  float temperature;                //!< Temperature of the gas
...
public:
 ...
 private:
  ...
  float n_HI;
  float n_HII;
  float temperature;                //!< temperature in site
