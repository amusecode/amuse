...
class mameclot(LiteratureReferencesMixIn):
    ...
    def result(self):
        ...
        if not self.convert_nbody is None:
            parts = datamodel.ParticlesWithUnitsConverted(result, self.convert_nbody.as_converter_from_si_to_generic())
            parts = parts.copy()
...
if __name__=="__main__":
    ...
    print cluster.total_mass()
    print len(cluster)
    
    pyplot.plot(cluster.x.number,cluster.y.number,"r.")
    pyplot.show()
