...
static inline void certifiedAdd(const double *x, const double *y, double *r){
...
template <int K, int L, int R>
static inline void baileyAdd_fast(double const *x, double const *y, double *z){
...
template <int K, int L, int R>
static inline void baileyAdd_renorm(double const *x, double const *y, double *z){
  if(L == 1) renorm2L_4Add1<K,R>(x, y[0], z);
  else{
	  int i, n;
	  double e, f[R+1];
