...
static inline float fast_two_sum(const float a, const float b, float &err){
...
/* Computes fl(a-b) and err(a-b). Assumes |a| >= |b| */
static inline float fast_two_diff(const float a, const float b, float &err){
  float s = FPadd_rn(a, -b);
...
static inline float two_sum(const float a, const float b, float &err){
  float s = FPadd_rn(a, b);
...
/* Computes fl(a-b) and err(a-b). */
static inline float two_diff(const float a, const float b, float &err){
  float s = FPadd_rn(a, -b);
...
/* Computes fl(a*b) and err(a*b). */
static inline float two_prod(const float a, const float b, float &err){
