		  ...
		  && SphP[i].Hsml > (1.01 * All.MinGasHsml)))
		{
		  ...
		  if(iter >= MAXITER - 10)
			...
			("i=%d task=%d ID=%d Hsml=%g Left=%g Right=%g Ngbs=%g Right-Left=%g\n   pos=(%g|%g|%g)\n",
			 ...
			 (double) SphP[i].NumNgb, SphP[i].Right - SphP[i].Left, P[i].Pos[0], P[i].Pos[1],
			 P[i].Pos[2]);
		      fflush(stdout);
		    }
		  ...
		  if(SphP[i].Right > 0 && SphP[i].Left > 0)
			...
			("** i=%d task=%d ID=%d Hsml=%g Left=%g Right=%g Ngbs=%g dhsml=%g\n ",
			 i, ThisTask, (int) P[i].ID, SphP[i].Hsml, SphP[i].Left, SphP[i].Right,
			 (double) SphP[i].NumNgb, SphP[i].DhsmlDensityFactor);
		      fflush(stdout);
		    }
 ...
 *  buffer.
 */
void gadgetmg2::density_evaluate(int target, int mode)
{
 ...
 */
int gadgetmg2::dens_compare_key(const void *a, const void *b)
{
