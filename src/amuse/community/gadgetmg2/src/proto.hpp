...
static double RndTable[RNDTABLE]; /*!< Hold a table with random numbers, refreshed every timestep */


static double DomainCorner[3];    /*!< gives the lower left corner of simulation volume */
static double DomainCenter[3];    /*!< gives the center of simulation volume */
static double DomainLen;          /*!< gives the (maximum) side-length of simulation volume */
static double DomainFac;          /*!< factor used for converting particle coordinates to a Peano-Hilbert mesh covering the simulation volume */
...
static double *DomainWork;        /*!< a table that gives the total "work" due to the particles stored by each processor */
...
static double  *DomainTreeNodeLen; /*!< this table gives for each leaf of the top-level tree the side-length of the corresponding node of the gravitational tree */
static double  *DomainHmax;        /*!< this table gives for each leaf of the top-level tree the maximum SPH smoothing length among the particles of the corresponding node of the gravitational tree */

static struct DomainNODE
...
static double TimeOfLastTreeConstruction; /*!< holds what it says, only used in connection with FORCETEST */
...
static double DriftTable[DRIFT_TABLE_LENGTH];      /*!< table for the cosmological drift factors */
static double GravKickTable[DRIFT_TABLE_LENGTH];   /*!< table for the cosmological kick factor for gravitational forces */
static double HydroKickTable[DRIFT_TABLE_LENGTH];  /*!< table for the cosmological kick factor for hydrodynmical forces */
 ...
 * variables.
 */
static struct sph_particle_data
...
static void   ewald_corr(double dx, double dy, double dz, double *fper);
static void   ewald_force(int ii, int jj, int kk, double x[3], double force[3]);
static void   ewald_init(void);
static double ewald_pot_corr(double dx, double dy, double dz);
static double ewald_psi(double x[3]);

//static void   fill_Tab_IO_Labels(void);
//static void   fill_write_buffer(enum iofields blocknr, int *pindex, int pc, int type);
static void   find_dt_displacement_constraint(double hfac);
...
static int    force_treeevaluate_ewald_correction(int target, int mode, double pos_x, double pos_y, double pos_z, double aold);
...
static double get_drift_factor(int time0, int time1);
static double get_gravkick_factor(int time0, int time1);
static double get_hydrokick_factor(int time0, int time1);
//static int    get_particles_in_block(enum iofields blocknr, int *typelist);
static double get_random_number(int id);
static int    get_timestep(int p, double *a, int flag);
...
static int    ngb_clear_buf(double searchcenter[3], double hguess, int numngb);
static void   ngb_treeallocate(int npart);
static void   ngb_treebuild(void);
static int    ngb_treefind_pairs(double searchcenter[3], double hsml, int *startnode);
static int    ngb_treefind_variable(double searchcenter[3], double hguess, int *startnode);
...
#ifdef NOPOW
static double pow(double, double);  /* on some old DEC Alphas, the correct prototype for pow() is missing, even when math.h is included */
#endif
...
//static void   readjust_timebase(double TimeMax_old, double TimeMax_new);
...
#endif
static void hydro_state_evaluate(double h, double pos[3], double vel[3], double *numngb,
    double *dhsml_out, double *rho_out, double *rhov_out, double *rhov2_out, double
    *rhoe_out);
...
static void hydro_state_at_point(double pos[3], double vel[3], double *h_out, double *ngb_out,
    double *dhsml_out, double *rho_out, double *rhov_out, double *rhov2_out, double *rhoe_out);
