...
void gadgetmg2::advance_and_find_timesteps(void)
{
 ...
 */
int gadgetmg2::get_timestep(int p,         /*!< particle index */
 ...
 */
void gadgetmg2::find_dt_displacement_constraint(double hfac /*!<  should be  a^2*H(a)  */ )
{
  int i, j, type, *temp;
  int count[6];
...
#ifdef TIMESTEP_LIMITER
int gadgetmg2::time_limiter_evaluate(int target, int mode)
{
 ...
 */
int gadgetmg2::time_compare_key(const void *a, const void *b)
{
...
void gadgetmg2::make_it_active(int target)
{
  int k;
  int tstart, tend;
