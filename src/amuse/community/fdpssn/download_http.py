...
if sys.version_info <= (3, 0):
    from urllib import (
        FancyURLopener, splithost, splitquery, splitattr,
...
class GetCodeFromHttp(object):
    ...
    def start(self):
        ...
        if sys.version_info <= (3, 0):
            ...
            filename, httpmessage, code = opener.retrieve(
                url, filename=os.path.join(self.src_directory(), filename))
            if code == 404:
                os.remove(filename)
            ...
            filename, headers = urlretrieve(
                url, os.path.join(self.src_directory(), filename))

        print("downloading finished")
        self.unpack_downloaded_file(filename)


def main(must_download_from_svn=False, version='5.0b'):
