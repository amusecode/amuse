...
{ \
const DOUBLE C1 = ((DOUBLE)  0.21375583945878254555518066964857q ); \
const DOUBLE C2 = ((DOUBLE)  0.18329381407425713911385974425217q ); \
const DOUBLE C3 = ((DOUBLE)  0.17692819473098943794898811709929q ); \
const DOUBLE C4 = ((DOUBLE) -0.44329082681170215849622829626258q ); \
const DOUBLE C5 = ((DOUBLE)  0.11728560432865935385403585669136q ); \
const DOUBLE C6 = ((DOUBLE)  0.50405474843802736404832781714239q ); \
EVOLVEA(C1*dt/2) \
EVOLVEB(C1*dt) \
EVOLVEA((C1+C2)*dt/2) \
EVOLVEB(C2*dt) \
...
{ \
const DOUBLE C1 = ((DOUBLE)  0.13861930854051695245808013042625q ); \
const DOUBLE C2 = ((DOUBLE)  0.13346562851074760407046858832209q ); \
const DOUBLE C3 = ((DOUBLE)  0.13070531011449225190542755785015q ); \
const DOUBLE C4 = ((DOUBLE)  0.12961893756907034772505366537091q ); \
const DOUBLE C5 = ((DOUBLE) -0.35000324893920896516170830911323q ); \
const DOUBLE C6 = ((DOUBLE)  0.11805530653002387170273438954049q ); \
const DOUBLE C7 = ((DOUBLE)  0.39907751534871587459988795520665q ); \
EVOLVEA(C1*dt/2) \
EVOLVEB(C1*dt) \
EVOLVEA((C1+C2)*dt/2) \
EVOLVEB(C2*dt) \
...
{ \
const DOUBLE C1  = ((DOUBLE)  0.10647728984550031823931967854896q ); \
const DOUBLE C2  = ((DOUBLE)  0.10837408645835726397433410591546q ); \
const DOUBLE C3  = ((DOUBLE)  0.35337821052654342419534541324080q ); \
const DOUBLE C4  = ((DOUBLE) -0.23341414023165082198780281128319q ); \
const DOUBLE C5  = ((DOUBLE) -0.24445266791528841269462171413216q ); \
const DOUBLE C6  = ((DOUBLE)  0.11317848435755633314700952515599q ); \
const DOUBLE C7  = ((DOUBLE)  0.11892905625000350062692972283951q ); \
const DOUBLE C8  = ((DOUBLE)  0.12603912321825988140305670268365q ); \
const DOUBLE C9  = ((DOUBLE)  0.12581718736176041804392391641587q ); \
const DOUBLE C10 = ((DOUBLE)  0.11699135019217642180722881433533q ); \
const DOUBLE C11 = ((DOUBLE) -0.38263596012643665350944670744040q ); \
EVOLVEA(C1*dt/2) \
EVOLVEB(C1*dt) \
EVOLVEA((C1+C2)*dt/2) \
EVOLVEB(C2*dt) \
...
{ \
const DOUBLE C1  = ((DOUBLE)  0.078795722521686419263907679337684q ); \
const DOUBLE C2  = ((DOUBLE)  0.31309610341510852776481247192647q ); \
const DOUBLE C3  = ((DOUBLE)  0.027918383235078066109520273275299q ); \
const DOUBLE C4  = ((DOUBLE) -0.22959284159390709415121339679655q ); \
const DOUBLE C5  = ((DOUBLE)  0.13096206107716486317465685927961q ); \
const DOUBLE C6  = ((DOUBLE) -0.26973340565451071434460973222411q ); \
const DOUBLE C7  = ((DOUBLE)  0.074973343155891435666137105641410q ); \
const DOUBLE C8  = ((DOUBLE)  0.11199342399981020488957508073640q ); \
const DOUBLE C9  = ((DOUBLE)  0.36613344954622675119314812353150q ); \
const DOUBLE C10 = ((DOUBLE) -0.39910563013603589787862981058340q ); \
const DOUBLE C11 = ((DOUBLE)  0.10308739852747107731580277001372q ); \
const DOUBLE C12 = ((DOUBLE)  0.41143087395589023782070411897608q ); \
const DOUBLE C13 = ((DOUBLE) -0.0048663605831352617621956593099771q ); \
const DOUBLE C14 = ((DOUBLE) -0.39203335370863990644808193642610q ); \
const DOUBLE C15 = ((DOUBLE)  0.051942502962449647037182904015976q ); \
const DOUBLE C16 = ((DOUBLE)  0.050665090759924496335874344156866q ); \
const DOUBLE C17 = ((DOUBLE)  0.049674370639729879054568800279461q ); \
const DOUBLE C18 = ((DOUBLE)  0.049317735759594537917680008339338q ); \
EVOLVEA(C1*dt/2) \
EVOLVEB(C1*dt) \
EVOLVEA((C1+C2)*dt/2) \
EVOLVEB(C2*dt) \
