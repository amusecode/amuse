...
/*
 * 03.03.2009: JPP
 * Moved numNeigh from vertex to site
 *
 * 24.02.2009: JPP
 * Added ballistic weights
*/
...
class Site : public Vertex {
  ...
  public:
    ...
    const unsigned int& get_neighId(unsigned int& i) const{ return neighId[i]; }
    ...
    void create_intensityIn( const unsigned int& directions ){ 
      intensityIn = new float[directions];
      for( unsigned int j=0; j<directions; j++ ){
	intensityIn[j] = 0.0;
      }
    }  
    //! store
    void set_intensityIn(const unsigned int& j, const float& in){ intensityIn[j]=in; }
    //! get the id of neighbour of site
    const float& get_intensityIn(unsigned int& i) const{ return intensityIn[i]; }

    //! delete the intensityIn array 
    void delete_intensityOut(){
      if(intensityOut){
	delete [] intensityOut;
	intensityOut = NULL;
      }
    ...
    void create_intensityOut( const unsigned int& directions ){ 
      intensityOut = new float[directions]; 
      for( unsigned int j=0; j<directions; j++ ){
	intensityOut[j] = 0.0;
      }
    ...
    //! store
    void set_intensityOut(const unsigned int& j, const float& out){ intensityOut[j]=out; }
    //! get the id of neighbour of site
    const float& get_intensityOut(unsigned int& i) const{ return intensityOut[i]; }
    ...
    unsigned char& get_outgoing( unsigned int& entry ){ return outgoing[entry]; }

    //! add the intensity from neighbour in ingoing intensity array
    void addRadiationDiffIn(const unsigned int& id, const double& intensity);
    //! add the intensity from neighbour in outgoing intensity array
    void addRadiationDiffOut(const unsigned int& id, const double& intensity);
  ...
  private:
    ...
    //float numberDensity;         //!< Number density in site
    //float ion_frac;              //!< Ionised fraction in site
...
class Site_Update{
  ...
  public:
    ...
    /* //! assign the total number of atoms to site */
    /* void set_ionised_fraction(const float& frac){ ion_frac = frac; } */
    /* //! get the total number of atoms the site */
