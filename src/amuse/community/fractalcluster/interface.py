...
class FractalClusterInterface(CodeInterface, LiteratureReferencesMixIn):
    """
    Makes a fractal distribution of N particles of dimension fdim, using ndiv
...
class FractalCluster(CommonCode):
    ...
    def __init__(self, unit_converter=None, **options):
        ...
        InCodeComponentImplementation.__init__(
            self, FractalClusterInterface(**options), **options
        )
    ...
    def define_methods(self, handler):
        ...
        handler.add_method(
            "get_number_of_particles_updated",
            (),
            (
                handler.NO_UNIT,
                handler.ERROR_CODE,
        ...
        handler.add_method(
            "set_target_number_of_particles", (handler.NO_UNIT,), (handler.ERROR_CODE,)
        )
        ...
        handler.add_method(
            "set_fractal_dimension", (handler.NO_UNIT,), (handler.ERROR_CODE,)
        )
...
class MakeFractalCluster:
    ...
    def result(self):
        ...
        if self.convert_nbody is not None:
            return datamodel.ParticlesWithUnitsConverted(
                particles, self.convert_nbody.as_converter_from_si_to_generic()
            ).copy()
        return particles
