...
! ***********************************************************************
    ...
    subroutine load_saved_model(id, filename, ierr)
        ...
        s% job% load_model_filename = trim(filename)
...
! ***********************************************************************
    ...
    subroutine update_star_job(id, ierr)
        ...
        call do_star_job_controls_after(id, s, .false., .false., ierr)
...
! ***********************************************************************
    ...
    subroutine do_evolve_for_one_step(id, result, ierr)
        integer, intent(in) :: id
        integer, intent(out) :: ierr, result
        type (star_info), pointer :: s  
        logical :: continue_evolve_loop
        logical,parameter :: dbg=.false.
        ...
        result = -1
        if(continue_evolve_loop) result = 0

    end subroutine do_evolve_for_one_step
...
! ***********************************************************************
    ...
    subroutine relax_to_new_entropy(id, xqs, temperature, rho, ierr)
        ...
        allocate(entropy(num_pts),d_dxa(num_eos_d_dxa_results,s%species))
