!RTpress patch:
! With the default SN time of 10 Myr, the SNe are out of sync
! with radiation feedback, since the radiation goes down by
! orders of magnitude at ~5 Myr. So here we reduce the SN time
! to 5 Myr.
!################################################################
!################################################################
!################################################################
...
subroutine feedbk(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  if(use_proper_time)then
     t0=5.*1d6*(365.*24.*3600.)/(scale_t/aexp**2)    !RTpress
     !t0=10.*1d6*(365.*24.*3600.)/(scale_t/aexp**2)  !RTpress
     current_time=texp
  else
     t0=5.*1d6*(365.*24.*3600.)/scale_t              !RTpress
