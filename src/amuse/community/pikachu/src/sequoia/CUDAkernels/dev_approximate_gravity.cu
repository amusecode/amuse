...
// float ds2_min = 1.0e10f;
  ...
  int *sh_Nngb = (int*)&sh_ngb[DIM]; // by M.I.
  ...
  for (int root_node = node_begend.x; root_node < node_begend.y; root_node += DIM) {
	...
	***/
	...
	//Check if a cell has to be opened
	  ...
	  bool   split   = split_node_grav_impbh_rsearch(nodeCOM, node_pos, nodeSize, group_pos, groupSize, group_eps, node_eps_val, rsearch_sq); // by M.I.
          #else
	  //bool   split   = split_node_grav_impbh(nodeCOM, group_pos, groupSize);
	  bool   split   = split_node_grav_impbh_rsearch(nodeCOM, node_pos, nodeSize, group_pos, groupSize, rsearch_sq); // by M.I.
          #endif
          ...
          //Minimum distance method
            ...
            bool   split   = split_node_grav_md_rsearch(node_pos, nodeSize, group_pos, groupSize, group_eps, node_eps_val, rsearch_sq);  // modified by M.I.
          #else
            //bool   split   = split_node_grav_md(node_pos, nodeSize, group_pos, groupSize);  //Check if node should be split
            bool   split   = split_node_grav_md_rsearch(node_pos, nodeSize, group_pos, groupSize, rsearch_sq);  // modified by M.I.
          #endif
	#endif
...
// 	      if (body_i != sh_jid[offs + j]) //If statement replaced by multiplication
		...
		acc_i = add_acc(acc_i, pos_i, sh_mass[offs + j], sh_pos[offs + j], sh_eps[offs + j], ds2, eps2, selfGrav, rsearch_sq, Nngb); // by M.I.
                #else
		//acc_i = add_acc(acc_i, pos_i, sh_mass[offs + j], sh_pos[offs + j], ds2, eps2, selfGrav);
		acc_i = add_acc(acc_i, pos_i, sh_mass[offs + j], sh_pos[offs + j], ds2, eps2, selfGrav, rsearch_sq, Nngb); // by M.I.
                #endif
#endif
...
//     float group_eps = computeGroupSoftening(body_vel, body_i, shmem);
    ...
    acc_i = approximate_gravity<blockDim2, 0>( DIM2x, DIM2y, tid, tx, ty,
					       ...
					       group_eps, body_vel, ds2, rsearch_sq, Nngb_i);
    if(apprCount < 0)
    {
    ...
    //acc_i.w = ds2; // commented out by M.I.
