...
subroutine gaslist
  ...
  real :: lpres,lrho,l_en,lvel
  real, save :: maxrho=-1.,maxp=-1.,maxen=-1.,maxvel=-1.
  integer :: i,j,k,step
  ...
  do i=1,meshx
    do j=1,meshy
      do k=1,meshz
        ...
        l_en=state(i,j,k,EN)
        lpres=gamma1*(l_en-0.5*lrho*lvel)
        maxen=max(l_en,maxen)
        maxvel=max(maxvel,lvel)
        maxp=max(lpres,maxp)
        maxrho=max(lrho,maxrho)
      enddo
    enddo
  enddo      
  step=max(meshx*meshy/NMAX,1)
  if(.NOT.allocated(state1)) return
  ...
  do i=1,meshx
    ...
    l_en=state(i,j,k,EN)
    lpres=gamma1*(l_en-0.5*lrho*lvel)
    select case (colorgas)
    case (1)
      red=lpres/maxp
      green=1-red
      blue=0.
      alpha_c=0.05+0.5*state(i,j,k,RHO)/maxrho
    case (2)
      red=l_en/maxen
      green=1-red
      blue=0.
...
subroutine createsubmenus(i,j,k)
 ...
 call glutaddmenuentry('show pres', 1)
 call glutaddmenuentry('show int en', 2)
 call glutaddmenuentry('show vel', 3)
