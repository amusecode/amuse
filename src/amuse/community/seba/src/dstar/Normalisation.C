...
//    seba_counters* new_seba_counters = new seba_counters;
    ...
    extern char *poptarg;
    int c;
    char* param_string = "n:N:RDSM:m:x:F:f:A:a:y:G:g:E:e:v:U:u:Q:q:T:I:w:P:p:n:s:z:";
...
//    cerr << "random number generator seed = " << actual_seed << endl;
   ...
   "         alpha  = %3.1f\n         lambda = %3.1f\n         beta   = %3.1f\n         gamma  = %4.2f",
	...
	mkrandom_binary(m_min_std, m_max_std, mf, m_exp,
...
//PRC(m_prim);PRC(m_min);	PRC(q_min2);PRC(q_min);PRL(q_max);
    ...
    cout << "In total mass spectrum (0.1 -- 100 Msun): " <<endl;
