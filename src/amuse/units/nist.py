...
class GetConstantsFromFiles(object):
    ...
    def compare_char_by_char(self):
        ...
        mydiff = difflib.unified_diff(
                self.nist_table.splitlines(1),
                self.local_table.splitlines(1),
                )
...
class Constants(object):
    ...
    def list_constants(self):
        ...
        namestr_length = (
                len(firstline)
                - len(firstline[firstline.find("   "):].lstrip())
                )
        column_index_of_uncertainty = (
                ...
                - len(
                    firstline[
                        namestr_length
                        + 21
                        + firstline[namestr_length+21:].find(" "):
                        ].lstrip()
                    )
                )
        column_index_of_unit = (
                len(firstline)
                - len(
                    firstline[
                        column_index_of_uncertainty
        ...
        for i in lines[number_of_header_lines:]:
            ...
            self.nisttable.append(
                    [
                        new_name,
                        float(
                            i[namestr_length:marker1].replace(
                                ' ', '',
                                ).replace('...', '')
                            ), unitstr
                        ]
