...
#            return factor_unit(1.0 / other, self)
    ...
    def __ne__(self, other):
        if isinstance(other, unit):
            if (
                    isinstance(self, base_unit)
                    and isinstance(other, base_unit)
                    ) or (
                    isinstance(self, nonnumeric_unit)
                    or isinstance(other, nonnumeric_unit)
                    ):
    ...
    def conversion_factor_from(self, x):
        if x.base is None:
            ...
            return (
                    1*(this_factor == other_factor)
                    or this_factor / other_factor
                    )
        else:
    ...
    def as_quantity_in(self, unit):
        ...
        if isinstance(unit, quantities.Quantity):
            raise exceptions.AmuseException(
                    "Cannot expres a unit in a quantity")
        else:
            ...
            if sub1 is not None:
                if sub2 is not None:
                    if sub1[1] == sub2[1]:
                        ...
                        raise exceptions.AmuseException(
                                """Cannot combine units from different systems:
                                {0} and {1}""".format(sub1[1], sub2[1]))
                else:
    ...
    def to_array_of_floats(self):
        """Represent a unit as an array of 8 64-bit floats. First float
        represents the factor, the other 7 the power of each base unit.  Cannot
        be used for non numeric units
        """
...
class base_unit(unit):
    """
    base_unit objects are orthogonal, indivisible units of a sytem of units.
    ...
    def reference_string(self):
        return '{0}.base({1!r})'.format(
                self.system.reference_string(), self.quantity)
...
class nonnumeric_unit(unit):
    ...
    def __mul__(self, other):
        if other == 1:
            return self
        raise exceptions.AmuseException(
                "Cannot derive other units from a non numeric unit")
...
class enumeration_unit(nonnumeric_unit):
    ...
    >>> my_unit = enumeration_unit(
            'my_unit','my_unit', [1,2,5], ["star","gas","planet"])
    >>> 2 | my_unit
    quantity<2 - gas>
    ...
    >>> my_unit = enumeration_unit(
            'my_unit','my_unit', None, ["star","gas","planet"])
    >>> 2 | my_unit
    quantity<2 - planet>
    >>> list(my_unit.quantities())
    ...
    """
    def __init__(
            self, name, symbol, possible_values=None, names_for_values=None):
        nonnumeric_unit.__init__(self, name, symbol)

        self.possible_values = self._initial_list_of_possible_values(
                possible_values, names_for_values)
        self.names_for_values = self._initial_names_for_values(
                possible_values, names_for_values)
        if not len(self.possible_values) == len(self.names_for_values):
            raise exceptions.AmuseException(
                    """Must provide equal lenght list for values({0}) and
                    names({1})""".format(
                        len(self.possible_values),
                        len(self.names_for_values)
                        )
                    )
        self.mapping_from_values_to_names = (
                self._initial_mapping_from_values_to_names()
                )
        self.DEFINED[name] = self

    def _initial_list_of_possible_values(
            self, possible_values, names_for_values):
        if possible_values is None:
            if names_for_values is None:
                raise exceptions.AmuseException(
                        """Must provide a list of values and / or a list of
    ...
    def __call__(self, string):
        ...
        if index > 0:
            ...
            raise exceptions.AmuseException(
                    """{0} is not a valid name for {1} enumeration
                    type""".format(string, self.name))
...
class factor_unit(derived_unit):
    ...
    def reference_string(self):
        return (
                '('
                + str(self.local_factor)
                + ' * '
                + self.local_unit.reference_string()
                + ')'
                )
...
class mul_unit(derived_unit):
    ...
    def reference_string(self):
        return (
                '('
                + self.left_hand.reference_string()
                + ' * '
                + self.right_hand.reference_string()
                + ')'
                )
    ...
    def base(self):
        return tuple(
            filter(
                lambda x: x[0] != 0,
                map(
                    lambda x: (x[0] * self.power, x[1]),
                    self.local_unit.base)
                )
            )
        ...
        return tuple(
                filter(
                    ...
                    map(
                        lambda x: (x[0] - x[1], x[2]),
                        self.combine_bases(
                            self.left_hand.base, self.right_hand. base)
                        )
                    )
                )
...
class IncompatibleUnitsException(exceptions.AmuseException):
    formatstring = """Cannot express {1} in {0}, the units do not have the same
    bases"""
