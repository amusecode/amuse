...
*
     ...
     &         abs(bin(k,2)).lt.tolm) go to 20 
            ...
            if(r(km).gt.1.d8) go to 20
            if(bin(k,8).lt.rlmin.or.bin(k,8).gt.rlmax) go to 20
*
            bin(k,8) = r(km)
            rbmin = xmin(im1)
            rbmax = xmax(im1)
*
            if(rbmin.gt.rlmax) go to 20
            if(rbmax.lt.rlmin) go to 20
*
            pbmin = rbmin
            if(rbmin.lt.rlmin) pbmin = rlmin
            pbmax = rbmax
            if(rbmax.gt.rlmax) pbmax = rlmax            
*
            do 50 kn=km+1,lmax
*
            im2 = iname(kn)
            if(ikind(im2).ne.2) go to 50
            kl = nwhich(im2)
*
            if(abs(body(im2)).lt.tolm) go to 50 
            if(abs(bin(kl,1)).lt.tolm.or.
     &         abs(bin(kl,2)).lt.tolm) go to 50   
            if(bin(kl,4).lt.0.0d0) go to 50
            if(bin(kl,6).lt.0.0d0) go to 50
            if(bin(kl,7).lt.0.0d0) go to 50
            if(r(kn).gt.1.d8) go to 50
            if(r(kn).lt.pbmin.or.r(kn).gt.pbmax) go to 50
*
*      compute probability of binary interaction and decide if interaction
*      will proceed
*
            ek = 0.5d0*(vr(im1)**2 + vt(im1)**2) + u(km)
            vek = 2.0d0*(ek - u(kn))
            vtek = vt(im1)*r(km)/r(kn)
            vrek = vek - vtek**2
            if(vrek.lt.0.0d0) then
              vrek = vtek/sqrt(2.0d0)
            else
              vrek = sqrt(vrek)
            endif
*
            pt = 2.0d0*(rbmax - rbmin)/(vrek + abs(vr(im1)))
            ap = bin(k,3)
            if(ap.lt.bin(kl,3)) ap = bin(kl,3)
            sm1234 = body(im1) + body(im2)
*
            call relvel(km,kn,ub)
            call densi(km,den)
*
            sig = 2.5d0*ap*sm1234/ub/ub
            pb3b3 = 0.5d0*co*sig*dt/pt/r(kn)**2
*
*    calculate probability of binary binary interaction using
*    procedure proposed by D.C. Heggie  (January 1998)
*
c            call period(km,ptd)
c            pb3b3d = 0.5d0*co*sig*ub*dt/ptd/vrek/r(kn)**2
*
            if(pb3b3.lt.ran2(irun)) go to 50
*
...
*            
     ...
     &            kmm,im1,idb,id1,id2,ik1,ik2sm1,sm2
            call flush(6)
*
            idb = nameb(im2)
            call get_loid(idb,id1)
     ...
     &            kn,im2,idb,id1,id2,ik1,ik2,sm1,sm2
            write(6,7543),im1,im2,sm1234o*zmbar,sm1234*zmbar,
     &                   sm1234n*zmbar,sm1234*zmbar - ss1234
 7543       format(1x,'im1,im2,sm1234o,sm1234,sm1234n,dmass =',2i9,
     &            1p4e14.6)
            call flush(6) 
*
...
*       
     ...
     &                im1,k,idb,id1,id2,ik1,ik2,sm1,sm2
                ...
                dmm = ssm12o - ssm12n
                write(6,5876) im1,idb,iexist,ik0n,apold,ap,apnew,rlo,
     &                        rln,dmm
 5876  format(1x,'1f im1,idb,iexist,ik0n,apold,ap,apnew,rlo,rln,dmm='
     &       ,2i9,2i4,1p6e14.6)
*
...
*
                ...
                if(iexist.eq.0) then
                  ...
                  cfr = body(im1)*zmbar
                  ssme = ssme + cfr
          print*,'yyy1 timevo,iime,cfr,ssme= ',timevo,iime,cfr,ssme
...
*
                if(dmm.le.1.d-10) then
                  ...
                  ehmlev = ehmlev - ehkin
                  smt = smt - ssevo
                  write(6,7321) im1,nob,nsevo,dmm,ssevo,ehkin
 7321          format(1x,'intb3b3a-evo im1,nob,nsevo,dmm,ssevo,ehkin=',
     &                  3I9,1p3e14.6)
                endif
*
              else
...
*       
     ...
     &                 im2,kl,idb,id1,id2,ik1,ik2,sm1,sm2
                ...
                call get_bs_type(idb,ik0n)
                dmm = ssm12o - ssm12n
             write(6,7875) im2,idb,iexist,ik0n,apold,ap,apnew,rlo,rln,
     &                     dmm
 7875  format(1x,'2f im2,idb,iexist,ik0n,apold,ap,apnew,rlo,rln,dmm='
     &       ,2i9,2i4,1p6e14.6)
*
*       check if semi major axis is greater than the sum of star radii
*       if not then the merger is due because of 4-body interaction.
...
*       evolution and due to 4-body interaction.
*
                if(ik1o.le.1.and.ik2o.le.1.or.iexist.eq.0) then
                  if(rad1+rad2.gt.ap) then
                   if(sm1+sm2.gt.sturn.and.(ik1.le.1.or.ik2.le.1)) then
...
*
                ...
                if(iexist.eq.0) then
                  ...
                  cfr = body(im2)*zmbar
                  ssme = ssme + cfr
          print*,'yyy2 timevo,iime,cfr,ssme= ',timevo,iime,cfr,ssme
...
*
                if(dmm.le.1.d-10) then
                  ...
                  ehmlev = ehmlev - ehkin
                  smt = smt - ssevo
                  write(6,7322) im2,nob,nsevo,dmm,ssevo,ehkin
 7322           format(1x,'intb3b3b-evo im2,nob,nsevo,dmm,ssevo,ehkin=',
     &                     3i9,1p3e14.6)
                  call flush(6)
                endif
              endif
...
*
              call kick(km,deb1)
              call kick(kn,deb2)
             write(6,*) 'b3b3<0.12 km,kn,im1,im2,deb=',km,kn,im1,im2,deb
              call flush(6)
*
              nb3b3 = nb3b3 + 1
              ehb3b3 = ehb3b3 + deb
...
*
                id3a = bin(k,7)/1.0d8
                iname(nt0) = nt0
                write(6,5445) im1,k,bin(k,7),id3,id3a
 5445           format(1x,'first binary disrupted: im1,k,bin,id3,id3a=',
     &              2i7,1pe22.14,2i7)
                id4a = bin(k,7) - 1.0d8*id3a
                write(6,5446) id4,id4a
 5446           format(1x,'id4,id4a =',2i7)
                call flush(6)
*
                ikind(im1) = -1
                ikind(nt0) = -1
                r(nt0) = r(km)*1.000001d0
...
*
     ...
     &                   bin(k,2)*zmbar
               call flush(6)
*
               write(43,440) id3,im1,timevo,sy1,sy1,rad1i,xlum1i,r(km),
...
*
                ...
                bin(kl,3) = 0.5d0*bin(kl,1)*bin(kl,2)/bin(kl,5)
                print*,'im2,kl,binold,binnew,apold,apnew,deb,m1,m2=',
     &                  im2,kl,binold,bin(kl,5),apold,
...
*
                ...
                ssm12o = sm1 + sm2
                ebold = bin(kl,5) 
                print*,'befo diss- im2,kl,idb,id1,id2,ik1,ik2,sm1,sm2=',
     &               im2,kl,idb,id1,id2,ik1,ik2,sm1,sm2
*        semi-major axis in R_sun unit
                ap = bin(kl,3)*rbar/rtidkg/rsuntopc
                call setSemi(idb,ap)
...
*       
                ...
                call get_loid_mass(idb,sm1)
                call get_hiid_mass(idb,sm2)
                print*,'afte diss- im2,kl,idb,id1,id2,ik1,ik2,sm1,sm2=',
     &                 im2,kl,idb,id1,id2,ik1,ik2,sm1,sm2
                bin(kl,1) = sm1/zmbar
                bin(kl,2) = sm2/zmbar
                bin(kl,7) = 1.0d8*float(id1) + float(id2)
                ...
                call get_bs_type(idb,ik0n)
                dmm = ssm12o - ssm12n
                write(6,9874) im2,idb,iexist,ik0n,apold,ap,apnew,rlo,
     &                     rln,dmm
 9874  format(1x,'1d im2,idb,iexist,ik0n,apold,ap,apnew,rlo,rln,dmm='
     &       ,2i9,2i4,1p6e14.6)
                call flush(6)
*
...
*
                  write(6,17) im2,im1,idb,id1,id2,ids,kl,inexch(im2)
...
*
                  ...
                  if(age1.ne.ages) then
                    print*,'b3b3a wrong ages-1  im1,im2,age1,ages =',
     &                     im1,im2,age1,ages               
                  endif
                  timea = timevo
*
...
*
       print*,'3b3b- exchange-1:kl,kn,km,imx,im2,idb,id1,id2,ids,names,'
...
*
                    ...
                    write(6,8875) im2,idb,iexist,sm12n*zmbar,
     &                            sm1d + sm2d,dmm
 8875         format(1x,'3b3b-creat1- im2,idb,iexist,sm12o,sm12n,dmm,',
     &        2i9,i4,1p5e14.6) 
...
*
                    ...
                    call binary_exists(idb,iexist)
                    dmm = sm12n*zmbar - (sm1d + sm2d)
                    write(6,7874) im2,idb,iexist,sm12n*zmbar,
     &                            sm1d + sm2d,dmm
 7874        format(1x,'3b3b-creat2- im2,idb,iexist,sm12o,sm12n,dmm,',
     &        2i9,i4,1p5e14.6) 
...
*
                ...
                if(abs(sm1+sm2-sm1d-sm2d).gt.1.d-10) then
                  write(6,5678) im2,idb,id1,id2,ik1,ik2,sm1,sm2,sm1d,
     &                        sm2d,sm1 + sm2 - (sm1d + sm2d)
 5678             format(1x,'wrong-diss-mass-2 im2,idb,id1,id2,ik1,ik2,'
...
*
                ...
                if(iexist.eq.0) then
                  ...
                  cfr = body(im2)*zmbar
                  ssme = ssme + cfr
            print*,'yyy3 timevo,iime,cfr,ssme= ',timevo,iime,cfr,ssme
...
*
                if(dmm.le.1.d-10) then
                  ...
                  ehmlev = ehmlev - ehkin
                  smt = smt - ssevo
                  write(6,9329) im2,nob,nsevo,dmm,ssevo,ehkin
 9329          format(1x,'intb3b3c-evo im2,nob,nsevo,dmm,ssevo,ehkin=',
     &                  3i9,1p3e14.6)
                endif
*
                ikind(im2) = -ikind(im2)
...
*
     ...
     &                      k,km,im1,names(im1),deb3,bin(k,5)
                write(6,*) 'nt,nn,nt0,im1,id3,id4 = ',nt,n+ndist+ndistm,
     &                      nt0,im1,id3,id4
                call flush(6)
                call kick(nt0,deb4)
                write(6,*) 'b3b3>0.12 nt0,id4,deb4=',nt0,names(nt0),
     &                      deb4
                call flush(6)
*
*      store information about binary binary interaction - second binary
...
*        between binaries removed due to escape (relaxation,
*        interaction) and destroied
*
                bin(k,4) = -bin(k,4)
*
              else
...
*
                ...
                call get_loid(idb,id3)
                call get_hiid(idb,id4)
                call get_loid_mass(idb,sm3)
                call get_hiid_mass(idb,sm4)
                call getRadius(id3,rad1i)
                call getRadius(id4,rad2i)
                call getLum(id3,xlum1i)
                call getLum(id4,xlum2i)
...
*
                id3a = bin(kl,7)/1.0d8
                write(6,8447) im2,kl,bin(kl,7),id3,id3a
 8447        format(1x,'second binary disrupted: im2,kl,bin,id3,id3a =',
     &             2i7,1pe22.14,2i7)
                id4a = bin(kl,7) - 1.0d8*id3a
                write(6,8448) id4,id4a
 8448           format(1x,'id4,id4a = ',2i7)
                call flush(6)
*
                iname(nt0) = nt0
                ikind(im2) = -1
                ikind(nt0) = -1
                r(nt0) = r(kn)*1.000001d0
                xmax(nt0) = 1.01d0*r(nt0)
                vr(nt0) = -vr(im2)
                vt(nt0) = -vt(im2)
*
...
*
     ...
     &                   bin(kl,2)*zmbar
                call flush(6)
*
                write(43,440) id3,im2,timevo,sy1,sy1,rad1i,xlum1i,r(kn),
     &                     ik11f,ik11f,ibstra(im2),ikind(im2)
                write(43,440) id4,nt0,timevo,sy2,sy2,rad2i,xlum2i,
     &                     r(nt0),ik21f,ik21f,ibstra(nt0),ikind(nt0)
...
*
                ...
                bin(k,3) = 0.5d0*bin(k,1)*bin(k,2)/bin(k,5)
                print*,'im1,k,binold,binnew,deb,apold,apnew,m1,m2=l',im1,
...
*
                ...
                ssm12o = sm1 + sm2
                ebold = bin(k,5)
                print*,'befor diss- im1,k,idb,id1,id2,ik1,ik2,sm1,sm2=',
     &               im1,k,idb,id1,id2,ik1,ik2,sm1,sm2
*        semi-major axis in R_sun unit
                ap = bin(k,3)*rbar/rtidkg/rsuntopc
                call setSemi(idb,ap)
...
*       
     ...
     &                 im1,k,idb,id1,id2,ik1,ik2,sm1,sm2
                ...
                call get_bs_type(idb,ik0n)
                dmm = ssm12o - ssm12n
                write(6,9873) im1,idb,iexist,ik0n,apold,ap,apnew,rlo,
     &                     rln,dmm
 9873    format(1x,'2d im1,idb,iexist,ik0n,apold,ap,apnew,rlo,rln,dmm='
     &       ,2i9,2i4,1p6e14.6)
*
*       check for exchange interaction
*
...
*
      print*,'3b3b-- id1a,id1,id2a,id2,ids,sm1,sm1s,sm2,sm2s,am3,sm3s,'
     &,'ub2,yy=',id1a,id1,id2a,id2,ids,sm1*zmbar,sm1s,sm2*zmbar,sm2s,
     &sm3*zmbar,sm3s,ub*ub,yy                    
*
*     make sure that id of stars and their masses are the same in the
*     evolution procedures (get_loid(idb,id1), get_loid_mass(idb,sm1s)) 
*     and in the bin(k,1), bin(k,2) and bin(k,7)
...
*
                  ...
                  call getAge(ids,ages)
                  if(age1.ne.ages) then
                    print*,'b3b3b wrong ages-1  im1,im2,age1,ages =',
     &                 im1,im2,age1,ages               
                  endif
                  timea = timevo
*
...
*
       print*,'3b3b--exchange-1:kl,kn,km,im1,imx,idb,id1,id2,ids,names,'
     &,'nameb,sm3,sm2,sm1,sm12o,sm12n,ebold,aold,anew,eold,enew,age1,'
...
*
                    ...
                    call binary_exists(idb,iexist)
                    dmm = sm12n*zmbar - (sm1d + sm2d)
                    write(6,7873) im1,idb,iexist,sm12n*zmbar,
     &                            sm1d + sm2d,dmm
 7873        format(1x,'3b3b-creat3- im1,idb,iexist,sm12o,sm12n,dmm,',
     &        2i9,i4,1p5e14.6)
                    if(iexist.eq.0) print*,'intb3b3-1 iexist=0'
*                                                
                    go to 52
...
*
                    anb = bin(k,3)*rbar/rtidkg/rsuntopc
*
      print*,'3b3b-- exchange-2:kl,kn,km,im1,imx,idb,id1,id2,ids,names,'
     &,'nameb,sm3,sm2,sm1,sm12o,sm12n,ebold,aold,anew,eold,enew,age1,'
     &,'ages,timea,,anb,anb-NB = ',kl,kn,km,im1,imx,idb,id1,id2,ids,
     &names(imx),nameb(im1),sm3*zmbar,sm2*zmbar,sm1*zmbar,
     &(sm1+sm2)*zmbar,sm12n*zmbar,ebold,aold,anew,eold,enew,age1,
...
*
                    ...
                    call binary_exists(idb,iexist)
                    dmm = sm12n*zmbar - (sm1d + sm2d)
                    write(6,7872) im1,idb,iexist,sm12n*zmbar,
     &                            sm1d + sm2d,dmm
 7872        format(1x,'3b3b-creat4- im1,idb,iexist,sm12o,sm12n,dmm,',
     &        2i9,i4,1p5e14.6)
                    if(iexist.eq.0) print*,'intb3b3-2 iexist=0'
*
                    go to 52
...
*
                ...
                call get_hiid_mass(idb,sm2d)
                if(abs(sm1+sm2-sm1d-sm2d).gt.1.d-10) then
                write(6,6678) im1,idb,id1,id2,ik1,ik2,sm1,sm2,sm1d,
     &                       sm2d,sm1 + sm2 - (sm1d + sm2d)
 6678           format(1x,'wrong-diss-mass-2 im1,idb,id1,id2,ik1,ik2,',
     &'sm1,sm2,sm1d,sm2d,dmmdis=',7i9,1p5e14.6)
                endif  
*
                call binary_exists(idb,iexist)
...
*
                ...
                if(iexist.eq.0) then
                  ...
                  cfr = body(im1)*zmbar
                  ssme = ssme + cfr
          print*,'yyy4 timevo,iime,cfr,ssme= ',timevo,iime,cfr,ssme
          print*,'merger binary interaction intb3b3-4 sm1,sm2,nmerge',
     &'im1,idb,iexist,ikind = ',sm1,sm2,nmerge,im1,idb,iexist,ikind(im1)
                  call flush(6)
*
                  if(sm1.le.tolm.and.sm2.le.tolm) then
*
...
*
                if(dmm.le.1.d-10) then
                  ...
                  ehmlev = ehmlev - ehkin
                  smt = smt - ssevo
                  write(6,1328) im1,nob,nsevo,dmm,ssevo,ehkin
 1328           format(1x,'intb3b3d-evo im1,nob,nsevo,dmm,ssevo,ehkin='
     &                 ,3i9,1p3e14.6)
                endif
*
                ikind(im1) = -ikind(im1)
...
*
*
      subroutine period(k,bper)
*
*
*       determine piriod of montion a binary in the cluster
*       ---------------------------------------------------
*       Method proposed by D.C. Heggie (January 1998)
*       ---------------------------------------------
*
*
      include 'common.h'
*
*
      real*8 e1,a1,q0,q1,gmin1,gmax1,rmin1,rmax1,s,u1,v1,drds,
     &       xcut,ycut,qmax,bper,rbinar(100),vbinar(100),delr

*
      real*4 ran2
*
      integer k,im1,im2,n,i,ipp,ibinar(100)
*
*
      n = nt
      i = 0
...
*
      ...
      a1 = r(k)*vt(im1)
*
*     determine 100 points on the orbit
*
      xcut = 0.01d0
      ycut = 1.0d0 - xcut
*
...
*
      ...
      call potent(0,1,n,rn1,u1)
      v1 = 2.0d0*(e1 - u1) - a1*a1/rn1/rn1
*
      if(v1.le.0.0d0) then
        go to 30
      endif
...
*
      if(q0.gt.q1) go to 30
*
      rbinar(i) = rn1
      vbinar(i) = v1
      ibinar(i) = i
*
      if(i.eq.100) go to 40
*
      go to 20
*
 40   continue
...
*
      bper = 0.0d0
*
*     sort positions according to increasing r
*
      call sort2(100,rbinar,ibinar)
*
      do 50 i = 2,100
         delr = rbinar(i) - rbinar(i-1)
         im1 = ibinar(i-1)
         im2 = ibinar(i)
         bper = bper + 0.5d0*(1.0d0/vbinar(im1) + 
     &                        1.0d0/vbinar(im2))*delr
 50   continue
*
      return
*
