...
@article{0004-637X-807-2-157,
 ...
 year={2015},
 abstract={The final phase of terrestrial planet formation is believed to begin
 with a large number of planetary on nearly circular, coplanar orbits. Mutual
 gravitational interactions gradually excite their eccentricities until their
 ...
 on well-separated, stable orbits. In this paper we explore a simple
 statistical model for the orbit distribution of planets formed by this
 process, based on the sheared-sheet approximation and the ansatz that the
 planets explore uniformly all of the stable region of phase space. The model
 provides analytic predictions for the distribution of eccentricities and
 semimajor axis differences, correlations between orbital elements of nearby
 planets, and the complete N -planet distribution function, in terms of a
 single parameter, the that is determined by the planetary masses. The
 predicted properties are generally consistent with N -body simulations of the
 giant-impact phase and with the distribution of semimajor axis differences in
 the Kepler catalog of extrasolar planets. A similar model may apply to the
 orbits of giant planets if these orbits are determined mainly by dynamical
 evolution after the planets have formed and the gas disk has disappeared.}
}
...
def get_mass_and_orbital_separation(
    ...
    mass = get_mass(
            a, surface_density_factor=surface_density_factor, mstar=mstar)
...
def new_planet_distribution(
    ...
    total_mass = mass.sum()
    iteration = 0
    while not is_hit(total_mass,  target_total_mass, accuracy):
        # print(iteration, total_mass / (1 | units.MEarth),
        #         current_surface_density_factor)
        iteration += 1
        ...
        current_surface_density_factor = (
                min_surface_density_factor
                + max_surface_density_factor
                ) / 2.0
...
def rotate(position, velocity, phi, theta, psi):  # theta and phi in radians
    r_unit = position.unit
    v_unit = velocity.unit
    matrix = new_rotation_matrix_from_euler_angles(phi, theta, psi)
    return (
            numpy.dot(matrix, position.value_in(r_unit)) | r_unit,
            numpy.dot(matrix, velocity.value_in(v_unit)) | v_unit,
            )
...
def posvel_from_orbital_elements(
        m_star, semimajor_axis, eccentricity, kepler, rng=None):
    if rng is None:
        rng = random
...
def make_planets(
    ...
    if theta is None:
        theta0 = numpy.radians(
                (rng.normal(-90.0, 90.0, 1)[0])
                )  # rotate under y
        theta0 = 0
...
def new_option_parser():
    ...
    result.add_option("-m", "--star-mass",
                      unit=units.MSun, type="float", default=1 | units.MSun,
                      dest="star_mass", help='mass of the star [%default]')
    result.add_option("-r", "--star-radius", unit=units.RSun, type="float",
                      default=1 | units.RSun, dest="star_radius",
                      help='radius of the star [%default]')
    result.add_option("--disk-r0", unit=units.AU, type="float",
                      default=1 | units.AU, dest="disk_minumum_radius",
                      help='minimum radius of the disk [%default]')
    result.add_option("--disk-r1", unit=units.AU, type="float",
                      default=100 | units.AU, dest="disk_maximum_radius",
                      help='maximum radius of the disk [%default]')
    result.add_option("--disk-mass", unit=units.MEarth, type="float",
                      default=100 | units.MEarth, dest="disk_mass",
                      help='mass of the disk [%default]')
    result.add_option("--planet-density", unit=units.g/units.cm**3,
                      type="float", default=3 | units.g/units.cm**3,
                      dest="planet_density",
                      help='density of the planets  (used for planet radii \
                              calculation) [%default]')
    result.add_option("--accurancy", type="float", default=0.0001,
                      dest="accurancy",
                      help='how accurate should the disk mass be integrated\
                      to[%default]')
...
def main(
    ...
    print("Total mass:", star.planets.mass.sum().as_quantity_in(units.MEarth))
    for i, planet in enumerate(star.planets):
        print(
                "Planet: {0: 3d}, mass: {1: 8.3f}  MEarth,\
                        a: {2: 8.2f} AU".format(
                            i,
