...
def relative_position_and_velocity_from_orbital_elements(
    ...
    for m2_i, a_i, ecc_i, ma_i in zip(
        ...
        kepler.initialize_from_elements(
                mass=(mass1+m2_i), semi=a_i, ecc=ecc_i, mean_anomaly=ma_i,
                random_orientation=-1)
        ri = kepler.get_separation_vector()
        vi = kepler.get_velocity_vector()
        # this is to get ~half of the orbits retrograde (that is with
        # inclination of 90--180 degrees) --> velocity = -velocity
        ...
        velocity_vectors.append(
                [
                    vel_orientation*vi[0],
                    vel_orientation*vi[1],
...
class SphericalIsotropicCloud(object):
    def __init__(
            self, target_number_of_particles, m_star=1. | units.MSun,
            m_cloud=0. | units.MSun, a_min=3000. | units.AU, a_max=10000. |
            units.AU, q_min=32. | units.AU, gamma=-1.5, seed=None):
    ...
    def new_model(self):
        ...
        a_in_au = random_power_min_max(
                self.targetN, self.a_min_q_corr.value_in(units.AU),
                self.a_max.value_in(units.AU), self.gamma+1.)
        a = a_in_au * 1. | units.AU
        ecc = ecc_random_power_with_min_peri(
                self.targetN, a, self.q_min, power=2.)
        ...
        position_vectors, velocity_vectors = (
                relative_position_and_velocity_from_orbital_elements(
                        self.m_star, m_comets, a, ecc, mean_anomaly,
                        seed=self.seed)
                )
...
def new_isotropic_cloud(
        number_of_particles, *list_arguments, **keyword_arguments):
    """
    Spherical isotropic cloud ~ Oort cloud given by distributions of orbital
    elements:
      semi-major axes -- power law, default: dn/da ~ a^(-1.5),
      eccentricities -- dn/de ~ e,
      constrain on the minimum pericenter,
    ...
    * Duncan, M.; Quinn, T.; Tremaine, S. --
    http://adsabs.harvard.edu/abs/1987AJ.....94.1330D
    * Feng, F.; Bailer-Jones, C. A. L. --
    http://adsabs.harvard.edu/abs/2014MNRAS.442.3653F (their DQT model)
    ...
    :argument gamma: exponent of the semimajor axis distribution, f(a) ~
    a^(gamma)
