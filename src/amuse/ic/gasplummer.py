...
class MakePlummerGasModel(object):
    ...
    def __init__(
            self, target_number_of_particles, convert_nbody=None,
            base_grid=None, rscale=1/1.695, mass_cutoff=0.999, do_scale=False):
        ...
        self.base_sphere = uniform_unit_sphere(
                target_number_of_particles, base_grid)

    def new_model(self):
        x, y, z = self.base_sphere.make_xyz()
        self.actual_number_of_particles = len(x)
        if self.actual_number_of_particles != self.target_number_of_particles:
            print("Warning: Plummer gas model has not the requested number of\
                    particles!")
    ...
    def result(self):
        ...
        result = datamodel.Particles(self.actual_number_of_particles)
