...
class MakeKingModel(object):
    def __init__(
            self, number_of_particles, W0, convert_nbody=None, do_scale=False,
    ...
    def gg_integral(self, y, g4_flag):
        # Array g contains the indefinite integral (from 0) of
        # y^2 exp(-y^2) dy,
        ...
        if (y >= self.YMAX):
            ...
            intyindx = int(yindx)
            g2 = (
                    self.g_integral[intyindx]
                    + (yindx-intyindx) * (
                        self.g_integral[intyindx+1]
                        - self.g_integral[intyindx]
                        )
                    )
        if (g4_flag):
    ...
    def get_dens_and_vel(self, psi, v2_flag):
        ...
        if (v2_flag):
            v2 = (
                    2
                    * (ep * g4 - 0.2 * self.scale_fac * y_max * p_max * p_max)
                    / dens
                    )
            return (dens, v2)
        else:
    ...
    def poisson(self):
        ...
        for i in range(1, nmax+1):
            ...
            (density, velocity_dispersion) = self.get_dens_and_vel(
                    self.psi[i], True)
            self.d.append(density)
            self.v2.append(velocity_dispersion)
            if (density < 0):
    ...
    def setpos(self):
        ...
        found_index = False
        for i1 in range(self.index[i], self.index[i+1]+2):
    ...
    def setvel(self, potential):
        ...
        # As with g[], the array indices run from 0 to NG, spanning a range 0
        # to YMAX, i.e. the cumulative distribution function for v is
        #
        # exp(-p) * g[i] - v33[i],
        #
        # where y = i*YMAX/NG (i = 0,...,NG) and v = sqrt(2)*sig*y (sig = 1
        # here).
        ...
        if (potential < -self.beta_w0):
            ...
            il = 0
            iu = int(((self.NG/self.YMAX) * math.sqrt(-potential)))
            # Binning OK for W0 < 16, *only* if beta >= 0.
            if (iu > self.NG):
                iu = self.NG
            rl = 0
            ...
            # 0 to NG*sqrt(-p)/YMAX.
            # Maximum possible value of v (for beta = 0) is the local escape
            # speed, sqrt(-2*p).
            v = (
                    (self.YMAX/self.NG)
                    * math.sqrt(2.0)
    ...
    def makeking(self):
        ...
        sig = math.sqrt(4.0 * math.pi * rho0 / 9.0)
        ...
        zmcore = (
                ...
                + (
                    zm[jcore+1] - zm[jcore]
                    ) * (1 - rr[jcore]) / (rr[jcore+1] - rr[jcore])
                )
        rhalf = (
                rr[jhalf]
                + (
                    rr[jhalf+1] - rr[jhalf]
                    ) * (0.5 - zm[jhalf]) / (zm[jhalf+1] - zm[jhalf])
                )
        ...
        masses = (
                numpy.zeros(self.number_of_particles)
...
def new_king_model(
        number_of_particles, W0, *list_arguments, **keyword_arguments):
    """
    Create a King model with the given number of particles and King
    dimensionless depth W0. Returns a set of particles with equal mass and
    positions and velocities distributed to fit a King distribution model. The
    model is centered around the origin. Positions and velocities are
    optionally scaled such that the kinetic and potential energies are 0.25 and
    -0.5 in nbody-units, respectively.

    :argument number_of_particles: Number of particles to include in the King
    model
    ...
    U=-0.5)
    :argument beta:  Steve's rescaling parameter (< 1) [0]. Models with b > 0
    are just rescaled King models; models with b < 0 approach isothermal
    spheres as b --> -infinity.
