...
This module contains functions used to generate realisations of the Salpeter
IMF.
"""
...
class SalpeterIMF(object):
    ...
    def mass(self, random_number):
        ...
        return (
                self.mass_min
                * (pow(1.0 + (factor * random_number), 1.0 / alpha1))
                )
...
def new_powerlaw_mass_distribution(
        number_of_particles, *list_arguments, **keyword_arguments):
    """Returns a salpeter mass distribution in SI units.

    :argument alpha: the dimensionless exponent of the Salpeter function
    (defaults to -2.35)
...
def new_powerlaw_mass_distribution_nbody(
    ...
    """
    if 'mass_min' not in keyword_arguments:
        keyword_arguments['mass_min'] = 0.1 | nbody_system.mass

    if 'mass_max' not in keyword_arguments:
