...
def resolve_close_encounter(time, bodies, Johannes):
    ...
    print(
        "Close encounter at t=", time.in_(
            units.Myr), "a=", a.in_(
            units.AU), "e=", e, "p=", p.in_(
                units.AU), "M=", bodies.mass.max().in_(
                    units.MSun), bodies.mass.min().in_(
                        units.MSun), "at d=", rcom.in_(
                            units.parsec), "with v=", vcom.in_(
                                units.kms))
...
def main(N, Rvir, Qvir, Fd):
    ...
    filename = 'Cl_N%g_R%gpc_Q%g_F%g.h5' % (
        N, Rvir.value_in(units.parsec), Qvir, Fd)
    t_end = 1.0 | units.Myr
    dt = 0.1 | units.Myr
    ...
    while time < t_end:
        ...
        while stopping_condition.is_set():
            ...
            for ci in range(len(stopping_condition.particles(0))):
                ...
                resolve_close_encounter(
                    gravity.model_time,
                    local_particles_in_encounter,
                    Johannes)
                Nenc += 1
                print(
                    "At time=",
                    gravity.model_time.value_in(
                        units.Myr),
                    "Nenc=",
                    Nenc,
                    "Rdisk=",
                    local_particles_in_encounter.disk_radius.in_(
                        units.AU))
