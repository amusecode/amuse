...
class MilkyWay_galaxy(object):
    ...
    def halo_potential(
            ...
            y,
            z,
            Mc=5.0E+10 | units.MSun,
            Rc=1.0 | units.kpc**2):
        r = (x**2 + y**2 + z**2).sqrt()
        rr = (r / Rc)
        return -constants.G * \
            (Mc / Rc) * (0.5 * numpy.log(1 + rr**2) + numpy.arctan(rr) / rr)

    def get_potential_at_point(self, eps, x, y, z):
        pot_disk = self.disk_and_bulge_potentials(
            x, y, z, 0.0 | units.kpc, 0.277 | units.kpc, 1.12E+10 | units.MSun)
        pot_bulge = self.disk_and_bulge_potentials(
            x, y, z, 3.7 | units.kpc, 0.20 | units.kpc, 8.07E+10 | units.MSun)
        pot_halo = self.halo_potential(
            x, y, z, Mc=5.0E+10 | units.MSun, Rc=6.0 | units.kpc)
...
def movie(time, sun_and_planets):
    ...
    R = [] | units.kpc
    R.append((sun_and_planets[1].position -
              sun_and_planets[0].position).length())
