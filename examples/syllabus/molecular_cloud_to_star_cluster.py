...
#    tend=2.0 | units.Myr
    ...
    expected_dt = 0.2 * numpy.pi * \
        numpy.power(eps, 1.5) / numpy.sqrt(constants.G * Mcloud / N)
    ...
    while ttarget < tend:
        ...
        E = sph.gas_particles.kinetic_energy() + sph.gas_particles.potential_energy() + \
...
def make_stars(cluster_particle):
    ...
    if N > 0:
        masses = new_salpeter_mass_distribution(
            N, 0.3 | units.MSun, min(
                100 | units.MSun, cluster_particle.mass))
...
#    gravity = bhtree(converter)
    ...
    channel_from_se_to_framework.copy_attributes(
