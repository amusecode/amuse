...
def new_disk_with_bump(Mstar=1 | units.MSun,
    ...
    bump.x += abump
    r_bump = abump
    inner_particles = bodies.select(
        lambda r: (
            com - r).length() < abump,
        ["position"])
    M_inner = inner_particles.mass.sum() + Mstar

    v_circ = (constants.G * M_inner * (2. / r_bump - 1. / abump)
...
def _irradiate_disk_with_pump(
    ...
    print("L=", stellar.particles[0].luminosity.in_(
        units.LSun), stellar.particles[0].temperature)
    ...
    if filename is None:
        ...
        ism = read_disk_with_bump(
            image_id=image_id,
            filename=filename)  # , Rmax=Rmax)
    ...
    while model_time < t_end:
        ...
        print("Date:" +
              ...
              str(localtime()[1]) +
              "." +
              str(localtime()[1]), "at", str(localtime()[3]) +
              "h", str(localtime()[4]) +
              "m")
