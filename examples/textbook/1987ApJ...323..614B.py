...
def collide_two_stars(t_end, distance, offset, velocity, nsteps):
    filename = "Hydro_AM06MSun.h5"
    try:
        pstar = read_set_from_file(filename, format='hdf5')
    except:
        from star_to_sph import star_to_sph
        mass = 0.6|units.MSun
        age = 8 | units.Gyr
        omega = 0|units.s**-1
        Nsph = 1000
        pstar, pcore, page = star_to_sph(mass, age, omega, Nsph)
    ...
    try:
        ...
        omega = 0|units.s**-1
        Nsph = 1000
        sstar, score, sage = star_to_sph(mass, age, omega, Nsph)
    print sstar
...
def new_option_parser():
    ...
    result.add_option("-v", unit=units.kms,
                      dest="velocity", type="float", default = 10|units.kms,
                      help="approach velocity [%default]")
    result.add_option("-n", 
...
if __name__ in ('__main__', '__plot__'):
    ...
    collide_two_stars(o.t_end, o.distance, o.offset, o.velocity, o.nsteps)
