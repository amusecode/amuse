...
#    m = star.get_temperature_profile(star.get_number_of_zones())
...
if __name__ == "__main__":
    ...
    for imass in range(len(mass)):
        ...
        pyplot.plot(L.value_in(units.LSun), rho.value_in(units.g/units.cm**3), lw=4, c=color[imass], label='$10M_\odot$', ls=ls[imass])
        mlim = dm
        for mi in range(len(m)):
            if m[mi]>mlim:
                mlim += dm
                print mi, len(L), len(rho)
                pyplot.scatter(L[mi].value_in(units.LSun), rho[mi].value_in(units.g/units.cm**3), c=color[imass], s=150, marker=symbol[imass], lw=0)
        pyplot.scatter(L[-1].value_in(units.LSun), rho[-1].value_in(units.g/units.cm**3), c=color[imass], s=150, marker=symbol[imass], lw=0)
