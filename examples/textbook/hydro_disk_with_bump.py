...
class RadHydro:
    def __init__(self, rad, hydro, star, disk):
        ...
        self.hydro = hydro(nbody_system.nbody_to_si(self.disk.mass.sum(), Rmax), number_of_workers=4)
...
#        self.hydro.parameters.timestep=1|units.day
        ...
        self.rad_to_disk = self.rad.gas_particles.new_channel_to(self.disk, attributes=["xion", "u"])
        self.star_to_rad = self.star.new_channel_to(self.rad.src_particles, attributes=["x", "y", "z"])
        self.disk_to_rad = self.disk.new_channel_to(self.rad.gas_particles, attributes=["x", "y", "z"])
...
#        print self.time, self.disk[0].position.in_(units.AU)
#        print "H:", self.time.value_in(units.day), self.hydro.gas_particles[0].position.in_(units.AU)
...
def new_disk_with_bump(Mstar = 10|units.MSun,
    ...
    disk = ProtoPlanetaryDisk(Ndisk, convert_nbody=converter, densitypower=1.5, Rmin=1, Rmax=Rmax/Rmin, q_out=1.0, discfraction=Mdisk/Mstar).result
    com = disk.center_of_mass()
    # determine bump's local velocity
    inner_particles = disk.select(lambda r: (com-r).length()<abump,["position"])
    M_inner = Mstar + inner_particles.mass.sum() 
    v_circ = (constants.G*M_inner*(2./abump - 1./abump)).sqrt().value_in(units.kms)
    #initialize bump
...
def hydro_disk_with_bump(Mstar = 10|units.MSun,
    ...
    star = evolve_star(Mstar, tstar)

    disk = new_disk_with_bump(star[0].mass, Ndisk, Mdisk, Rmin, Rmax, Mbump, Rbump, abump)
