...
def local_relax(gas_particles, hydro, gravity_field=None, monitor_func=check_energy_conservation,
        bridge_options=dict()):
    """
...
def generate_initial_conditions(
    ...
    masses = new_salpeter_mass_distribution(number_of_stars, mass_min=1|units.MSun)
...
def plot_hydro_and_stars(hydro, stars):
    ...
    cbar.set_label('$\log_{10} (\mathrm{n}) \,\,\, [\mathrm{amu/cm}^3]$', rotation=270)
    
    cm = pyplot.cm.get_cmap('RdBu')
    m = 50*numpy.log10(stars.mass/stars.mass.min())
    c = numpy.sqrt(stars.mass/stars.mass.max())
    pyplot.scatter(-stars.x.value_in(units.parsec), -stars.y.value_in(units.parsec), c=c, s=m, lw=0, cmap=cm)

    pyplot.savefig("plot_relaxed_gas_and_star")
#    pyplot.show()
