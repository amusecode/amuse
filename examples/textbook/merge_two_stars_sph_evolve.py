...
if __name__ == "__main__":
    ...
    pyplot.scatter(temperature[0].value_in(units.K), luminosity[0].value_in(units.LSun), c=color[0], s=150, marker="^")

    print "Evolve single star of M=", (Mprim).in_(units.MSun)+(0.2|units.MSun)
    time, stp, mass, radius, temperature, luminosity = evolve_single_star(Mprim+(0.2|units.MSun), tend)
    pyplot.plot(temperature.value_in(units.K), luminosity.value_in(units.LSun), c=color[1], lw=2)
    pyplot.scatter(temperature[0].value_in(units.K), luminosity[0].value_in(units.LSun), c=color[1], s=150, marker="^")

    print "Evolve single star of M=", (Mprim+Msec).in_(units.MSun) + (0.4|units.MSun)
    time, stp, mass, radius, temperature, luminosity = evolve_single_star(Mprim+(0.4|units.MSun), tend)
    pyplot.plot(temperature.value_in(units.K), luminosity.value_in(units.LSun), c=color[3], lw=2)
    pyplot.scatter(temperature[0].value_in(units.K), luminosity[0].value_in(units.LSun), c=color[3], s=150, marker="^")
    ...
    pyplot.scatter(temperature[n-1].value_in(units.K), luminosity[n-1].value_in(units.LSun), c=color[2], s=150, marker="o")
    pyplot.plot(temperature[n:].value_in(units.K), luminosity[n:].value_in(units.LSun), c=color[2])
    pyplot.scatter(temperature[n+1].value_in(units.K), luminosity[n+1].value_in(units.LSun), c=color[2], s=150, marker="o")
