...
def random_semimajor_axis_PPE(Mprim, Msec, P_min=10.|units.day, P_max=100.|units.yr):
...
def make_secondaries(center_of_masses, Nbin):
    ...
    for bi in binaries:
        mp = bi.mass
        ms = numpy.random.uniform(mmin.value_in(units.MSun), mp.value_in(units.MSun)) | units.MSun
