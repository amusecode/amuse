...
def get_amuse_directory_root():
    ...
    directory_of_this_script = os.path.dirname(
            os.path.dirname(filename_of_this_script))
...
if sys.hexversion > 0x03000000:
    def get_amuse_directory():
        ...
        directory_of_this_script = os.path.dirname(
                os.path.dirname(filename_of_this_script))
        directory_of_this_script = os.path.join(
                directory_of_this_script, 'build', 'lib')
        if os.path.isabs(directory_of_this_script):
            return directory_of_this_script
        else:
            return os.path.abspath(directory_of_this_script)
...
class ParseCommandLine(object):
    ...
    be used to create C++ or Fortran code to handle the MPI messages, create a
    header file or create stub code as a start for defining the interface
    between the code and AMUSE.
    ...
    def __init__(self):
        ...
        # hack to set the name, for reporting errors and help
        self.parser.prog = 'build.py'
        ...
        self.parser.add_option(
            ...
            default="c",
            dest="type",
            help="""TYPE of the code to generate. Can be one of c, h, H, f90,
            py, java or cython. <c> will generate c code. <h/H> will generate
            c/c++ header. <f90> will generate fortran 90 code. <py> will
            generate a python worker wrapper <java> will generate java
            interface or class, depending on mode. (Defaults to c)""")

        self.parser.add_option(
            "-m",
            "--mode",
            choices=[
                "mpi", "stub", "dir", "sockets", "interface", "class",
                "script"],
            default="mpi",
            dest="mode",
            help="""MODE of the code to generate. Can be <mpi>, <stub>,
            <dir>,<sockets>, <interface>, <class> or <script>. Generate the MPI
            handling code or STUB code for the link between mpi and the code
            (if needed). <dir> will create a directory ann populate it with the
            files needed to build a code. (Defaults to mpi)""")
        ...
        self.parser.add_option(
            ...
            default="",
            dest="ignore",
            help="""Name of the classes to ignore, functions defined on these
            classes will not generate code. Comma separated list""")
        ...
        self.parser.add_option(
            ...
            default="",
            dest="underscore",
            help="""Name of the classes to underscore the functions of, for XL
            fortran compilers""")
        ...
        self.parser.add_option(
            ...
            default="true",
            dest="needs_mpi",
            help="""If this boolean flag is set, the worker will initialize
            mpi, even in the sockets channel is used. Defaults to true""")
        ...
        self.parser.add_option(
            ...
            default="",
            dest="cython_import",
            help="""Name of the module to import for the cython worker (name of
            the .so file)""")
    ...
    def parse_options(self):
        ...
        if self.options.underscore:
            self.options.underscore_classes = list(
                    self.parse_underscore_classes()
                    )
        else:
        ...
        if self.options.mode == 'dir':
            ...
            try:
                ...
                if len(self.arguments) > 2:
                    self.options.name_of_implementation_class = (
                            self.arguments[2])
...
def make_file(settings):
    ...
    try:
        if settings.name_of_module_or_python_file.endswith('.py'):
            ...
            if settings.name_of_implementation_class is not None:
                implementation_class = module[
                        settings.name_of_implementation_class]
        else:
            module = __import__(
    ...
    try:
        ...
        builder.ignore_functions_from_specification_classes = (
                settings.ignore_classes)
        ...
        uc.show_error_and_exit(
                """'{0}' and '{1}' is not a valid combination of type and mode,
                cannot generate the code""".format(
                    settings.type,
                    settings.mode,
                    )
                )
