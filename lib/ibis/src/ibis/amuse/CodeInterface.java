...
public abstract class CodeInterface implements RegistryEventHandler, Runnable {
	...
	protected void sendInitReply(int callID) throws IOException {
		AmuseMessage result = new AmuseMessage(callID,
				AmuseMessage.FUNCTION_ID_INIT, 1);
	...
	protected void sendInitReply(int callID, Throwable error)
			throws IOException {
		AmuseMessage result = new AmuseMessage(callID,
				AmuseMessage.FUNCTION_ID_INIT, 1, error.getMessage());
	...
	public static void main(String[] arguments) {
		...
		try {
			...
			MPIProfilingCollector collector = new MPIProfilingCollector();

			poolInfo = new PoolInfo(id, nrOfNodes, collector.getPort());

			collector.setIbises(poolInfo.getWorkerIbisList(nrOfWorkers));

			if (poolInfo.getRank() == 0) {
				if (jni) {
					codeInterface = new JNICodeInterface(id, poolInfo, codeName);
				} else {
					codeInterface = new SocketCodeInterface(id, poolInfo,
							codeName, codeDir, amuseHome, mpiexec, mpdboot,
							nrOfWorkers);
				}
